<?php
/**
 * @package DTC
 * @version $Id: strings.php,v 1.23 2007/03/18 14:18:29 indivision Exp $
 * @abstract Arrays for translations admin panel to many laguages
 * 
 * adedd swedish to array by seeb <seeb@seeb.net.pl>
 */

$txt_dtcrm_your_domain_has_been_registred_elsewhere = array(
"fr" => "Votre nom de domaine a été enregistré ailleurs (eg pas sur ce site).
Pour demander un transfère ici, veuillez cliquez ",
"en" => "Your domain name has been registred elsewhere (i.e.
not on this site). To order its transfer, please click",
"hu" => "Az ön domainje valahol másol van regisztrálva (nem ezen a szerveren).
Ha szeretné áttranszferáltatni erre a szerverre, kattintson ide.",
"it" => "Il tuo nome a dominio è già stato registrato da qualche parte 
(non in questo sito). Per ordinare il suo trasferimento clicca",
"nl" => "Dit domein is elders geregistreerd. Klik voor een transfer",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúÊäÈëµÄÓòÃûÒÑ±»×¢²á¡£ÈçÐè¹ºÂò¸ÃÓòÃû(´ÓËüµÄËùÓÐÈËÄÇÀï¹ºÂò)£¬Çëµã»÷ÕâÀï",
"pl" => "Twoja domena zosta³a zarejestrowana na innym serwerze ",
"se" => "TRANS SVENSKA",
"es" => "Su dominio ha sido registrado con otro registrador. Para pedir el traslado, click aqu&iacute;",
"pt" => "O seu dominio foi registado noutro local. Para pedir a alteração clique "
);

$txt_dtcrm_if_you_want_to_keep_your_current_registrar = array(
"fr" => "Si vous voullez garder votre registrar actuel, vous devez faire pointer
le whois vers les DNS suivants :",
"en" => "If you want to keep your current registrar, you have to make the whois point
to these DNS:",
"hu" => "Ha meg szeretne maradni a jelenlegi regisztrátoránál, akkor a whois rekordnak
 azok",
"it" => "Se vuoi mantenere il tuo attuale registrar, devi far puntare 
il whois verso questi DNS:",
"nl" => "Om de huidige registrar te behouden dient de wijs dns naar het volgende adres",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Èç¹ûÄúÏ£Íû±£ÁôÏÖÓÐµÄ×¢²á£¬Äú±ØÐëÔÚÏÂÁÐ DNS µÇ¼ÇÄúµÄ whois ÐÅÏ¢",
"pl" => "Je¶li chcesz pozostawiæ aktualnego rejestratora, mo¿esz  to zrobiæ w zak³adce DNS:DTCRM ",
"se" => "TRANS SVENSKA",
"es" => "Si desea mantener el registrador actual debe apuntar la informaci&oacute;n WHOIS a estos DNS:",
"pt" => "Caso queira manter o seu actual Registar, queira fazer as seguintes alterações de DNS :"
);

$txt_dtcrm_primary_dns = array(
"fr" => "DNS primaire :",
"en" => "Primary DNS:",
"hu" => "Elsõdleges DNS:",
"it" => "DNS primario:",
"nl" => "Primaire DNS ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ê×Ñ¡DNS£º",
"pl" => "Podstawowy serwer nazw (pri DNS) ",
"se" => "TRANS SVENSKA",
"es" => "DNS primario",
"pt" => "DNS primário"
);

$txt_dtcrm_secondary_dns = array(
"fr" => "DNS secondaire :",
"en" => "Secondary DNS:",
"hu" => "Másodlagos DNS:",
"it" => "DNS secondario:",
"nl" => "Secondaire DNS ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "±¸ÓÃDNS£º",
"pl" => "Zapasowy Serwer nazw (sec DNS) ", 
"se" => "TRANS SVENSKA",
"es" => "DNS secundario",
"pt" => "DNS secundário"
);

$txt_dtcrm_your_domain_name_whois_data = array(
"fr" => "Vos données whois pour ce domaine :",
"en" => "Your domain name whois data:",
"hu" => "Az ön domain-jének whois adatai:",
"it" => "Dati per il whois del tuo dominio:",
"nl" => "Domeinnaam whois data ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúµÄÓòÃûµÄwhoisÐÅÏ¢£º",
"se" => "TRANS SVENSKA",
"pl" => "Dane <i>whois</i> twojej domeny",
"es" => "Datos WHOIS de su dominio",
"pt" => "Dados para o WHOIS do seu dominio"
);

$txt_dtcrm_what_subzone_do_you_want_to_use = array(
"fr" => "Quel sous-zone voulez vous utiliser (exemple : \"ns1\") :",
"en" => "What subzone do you want to use (example: \"ns1\"):",
"hu" => "Milyen alzónát szeretne használni (például: \"ns1\"):",
"it" => "Quale subzone vuoi usare (esempio: \"ns1\"):",
"nl" => "Welke subzone wil je gebruiken (bijvoorbeeld: \"ns1\"):",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúÏ£ÍûÊ¹ÓÃÊ²Ã´¶þ¼¶ÓòÃû(ÀýÈç: \"ns1\")£º",
"pl" => "Jakiej dodatkowej strefy chcesz u¿yæ?(np: \"ns1\") ",
"se" => "TRANS SVENSKA",
"es" => "¿Qué subzona desea usar? (ejemplo: &quot;ns1&quot;):",
"pt" => "Qual subzona quer usar (Exemplo: \"ns1\") :"
);

$txt_dtcrm_edit_a_name_server = array(
"fr" => "Editer un serveur de nom:",
"en" => "Edit a name server:",
"hu" => "Szerkeszti a névszerver adatait:",
"it" => "Edita un name server:",
"nl" => "Bewerk een Nameserver: ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "±à¼­ÓòÃû·þÎñÆ÷ÐÅÏ¢",
"pl" => "Edycja serwerów nazw",
"se" => "TRANS SVENSKA",
"es" => "Editar un servidor de nombres",
"pt" => "Editar servidores de nomes"
);

$txt_dtcrm_name_server_hostname = array(
"fr" => "Nom d'hote du serveur de nom : ",
"en" => "Name server hostname: ",
"hu" => "Névszerver gazdanév: ",
"it" => "Hostname del name server:",
"nl" => "Hostname van de server: ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÓòÃû·þÎñÆ÷µÄÓòÃû(²»ÊÇIP)",
"pl" => "Nazwa serwera nazw (nie IP) ",
"se" => "TRANS SVENSKA",
"es" => "nombre de host de servidor de nombres",
"pt" => "Nome do servidor \"hostname\" "
);

$txt_wrong_user_or_password_or_timeout_expire = array(
"fr" => "Mauvais utilisateur ou mot de passe, ou temps d'expiration atteint !",
"en" => "Wrong user or password, or timeout expired!",
"hu" => "Hibás felhasználó vagy jelszó vagy idõtúllépés!",
"it" => "Utente o password errati, o tempo scaduto!",
"nl" => "Verkeerde login of timeout expired!",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÓÃ»§Ãû»òÃÜÂë´íÎó£¬»òÕßÁ¬½Ó³¬Ê±£¡",
"pl" => "Z³e has³o lub nazwa u¿ytkownika, mo¿liwe przedawnienie has³a ",
"se" => "TRANS SVENSKA",
"es" => "Nombre de usuario o contrase&ntilde;a incorrectos o tiempo de ejecuci&oacute;n excedido",
"pt" => "Nome do utilizador ou palavra chave incorrecto" 
);

$txt_dtcrm_registration_of_name_server_successfull = array(
"fr" => "Enregistrement de votre serveur de nom réussit",
"en" => "Registration of your name server was succesful",
"hu" => "A névszerver regisztrációja sikeresen megtörtént.",
"it" => "Registrazione del tuo name server avvenuta con successo",
"nl" => "Registratie van nameserver successvol",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "×¢²áÓòÃû·þÎñÆ÷³É¹¦",
"pl" => "Rejestracja serwerów dns powiod³a siê ",
"se" => "TRANS SVENSKA",
"es" => "Registro del servidor de nombres correcto.",
"pt" => "O seu dominio foi registado correctamente"
);


$txt_dtcrm_registration_of_name_server_failed = array(
"fr" => "Echec lors de l'enregistrement de votre serveur de nom",
"en" => "Registration of your name server failed",
"hu" => "A névszerver regisztrációja sikertelen volt.",
"it" => "Registrazione del name server fallita",
"nl" => "Registratie van nameserver gefaald ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "×¢²áÓòÃû·þÎñÆ÷Ê§°Ü",
"pl" => "Rejestracja serwerów dns nie powiod³a siê ",
"se" => "TRANS SVENSKA",
"es" => "Registro de su servidor de nombres fallido",
"pt" => "O regisot do seu dominio falhou"
);

$txt_dtcrm_deletion_of_name_server_succesfull = array(
"fr" => "Effacement de votre serveur de nom réussit",
"en" => "Deletion of name server was succesful",
"hu" => "Sikeresen törölte a névszervert",
"it" => "Cancellazione del name server avvenuta con successo",
"nl" => "Nameserver successvol verwijderd",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "É¾³ýÓòÃû·þÎñÆ÷³É¹¦",
"pl" => "Deklaracja serwera nazw powiod³a siê ",
"se" => "TRANS SVENSKA",
"es" => "Eliminaci&oacute;n correcta del servidor de nombres",
"pt" => "O dominio foi eliminado"
);

$txt_dtcrm_edition_of_name_server_succesfull = array(
"fr" => "Edition de votre serveur de nom réussit",
"en" => "Addition of name server was succesfull",
"hu" => "A névszerver módosítása sikeresen megtörtént ",
"it" => "Editazione del name server avvenuta con successo",
"nl" => "Nameserver succesvol toegevoegd",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "±à¼­ÓòÃû·þÎñÆ÷ÐÅÏ¢³É¹¦",
"pl" => "Edycja serwera nazw powiod³a siê",
"se" => "TRANS SVENSKA",
"es" => "Edici&oacute;n del servidor de nombres correcta",
"pt" => "A alteração do servidor de nome foi efectuada correctamente" 
);

$txt_dtcrm_edition_of_name_server_failed = array(
"fr" => "Edition de votre serveur de nom réussit",
"en" => "Addition of name server failed",
"hu" => "A névszerver módosítása nem sikerült",
"it" => "Editazione del name server fallita",
"nl" => "Toevoegen van nameserver gefaald ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ìí¼ÓÓòÃû·þÎñÆ÷Ê§°Ü ",
"pl" => "Edycja serwera nazw nie powiod³a siê",
"se" => "TRANS SVENSKA",
"es" => "Edici&oacute;n del servidor de nombres fallida",
"pt" => "A alteração do servidor de nome falhou"
);

$txt_dtcrm_deletion_of_name_server_failed = array(
"fr" => "Echec lors de l'effacement du serveur de nom",
"en" => "Deletion of name server failed",
"hu" => "A névszerver törlése nem sikerült",
"it" => "Cancellazione del name server fallita",
"nl" => "Verwijderen van nameserver gefaald ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "É¾³ýÓòÃû·þÎñÆ÷Ê§°Ü",
"pl" => "Deklaracja serwera nazw nie powiod³a siê  ",
"se" => "TRANS SVENSKA",
"es" => "Eliminaci&oacute;n del servidor de nombres fallida",
"pt" => "A eliminação do servidor de nome falhou"
);

$txt_dtcrm_list_of_registred_name_server = array(
"fr" => "Liste de vos serveurs de nom enregistrés : ",
"en" => "List of your registred name-servers:",
"hu" => "A regisztrált névszerverek listája:",
"it" => "Lista dei tuoi name-server registrati:",
"nl" => "Lijst van jouw nameservers ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúÒÑ×¢²áµÄÓòÃû·þÎñÆ÷µÄÁÐ±í£º",
"pl" => "Lista Twoich serwerów DNS ",
"se" => "TRANS SVENSKA",
"es" => "Listado de sus servidores de nombres registrados",
"pt" => "lista dos servidores de nomes foi registada"
);

$txt_dtcrm_register_new_name_server = array(
"fr" => "Enregistrer un nouveau serveur de nom :",
"en" => "Register a new name server:",
"hu" => "Új névszerver regisztrálása: ",
"it" => "Registra un nuovo name server:",
"nl" => "registreer een nieuwe nameserver ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÐÂ×¢²áÒ»¸öÓòÃû·þÎñÆ÷£º",
"pl" => "Nowy serwer nazw: ",
"se" => "TRANS SVENSKA",
"es" => "Registrar un nuevo servidor de nombres",
"pt" => "Registar novo servidor de nomes" 
);

$txt_dtcrm_delete_name_server = array(
"fr" => "Effacer le serveur de nom",
"en" => "Delete name server",
"hu" => "Névszerver törlése",
"it" => "Cancella name server",
"nl" => "Verwijder nameserver",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "É¾³ýÓòÃû·þÎñÆ÷",
"pl" => "Usuñ serwer nazw ",
"se" => "TRANS SVENSKA",
"es" => "Eliminar servidor de nombres",
"pt" => "Eliminar servidor de nomes"
);

$txt_dtcrm_select_your_domain_name_for_adding_name_server = array(
"fr" => "Selectionner un de vos nom de domaine pour ajouter le serveur de nom a la registry :",
"en" => "Select one of your domain-names for registering an additional name server to the registries:",
"hu" => "Válasszon egy domain nevet amit regisztrálni szeretne névszerveren:",
"it" => "Seleziona uno dei tuoi nomi a domini per aggiungere un name 
server ai registri",
"nl" => "Selecteer een van je domeinnamen om een nieuwe nameserver to registreren tot de registry ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ñ¡ÔñÒ»¸öÓòÃû×¢²áÎª±¸ÓÃDNS·þÎñÆ÷£º",
"pl" => "Wybierz jeden z twoich DNS\'ow ",
"se" => "TRANS SVENSKA",
"es" => "Seleccione uno de sus nombres de dominio para a&ntilde;adir el servidor de nombres a los registros",
"pt" => "Selecionar um dominio para adicionar um servidor de nomes aos registos :"
);

$txt_dtcrm_ip_addr_of_that_name_server = array(
"fr" => "Addresse IP de ce serveur de nom :",
"en" => "IP address of name server:",
"hu" => "A névszerver IP címe:",
"it" => "Indirizzo IP di questo name server:",
"nl" => "IP van de nameserver ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÓòÃû·þÎñÆ÷µÄIPµØÖ·£º",
"pl" => "Adres IP serwera nazw (DNS)",
"se" => "TRANS SVENSKA",
"es" => "Direcci&oacute;n IP del servidor de nombres",
"pt" => "Endereço IP desse servidor de nomes :"
);

$txt_dtcrm_what_to_do = array(
"fr" => "Que voulez vous ajouter :",
"en" => "What do you want to add:",
"hu" => "Mit szeretne hozzáadni ?",
"it" => "Quale vuoi aggiungere?:",
"nl" => "Wat wil je toevoegen:",
"ru" => "TRANS What do you want to add:",
"de" => "TRANS What do you want to add:",
"zh" => "ÄúÏëÌí¼ÓÊ²Ã´£º",
"pl" => "Co chcesz dodaæ:",
"se" => "TRANS SVENSKA",
"es" => "¿Qu&eacute; desea a&ntilde;adir?:",
"pt" => "O que quer adicionar :"
);

$txt_dtcrm_hosting_and_domain_reg = array(
"fr" => "Hébergement + enregistrement ou transfère de nom de domaine",
"en" => "Hosting + domain name registration or transfer",
"hu" => "Hoszting + domain név regisztráció vagy transzfer",
"it" => "Hosting + registrazione nome a dominio o 
trasferimento",
"nl" => "Hosting + domeinnaam registratie of transfer",
"ru" => "TRANS Hosting + domain name registration or transfer",
"de" => "TRANS Hosting + domain name registration or transfer",
"zh" => "¿Õ¼ä£«ÓòÃû×¢²á»ò×ªÈÃ",
"pl" => "Hosting + rejestracja domeny ",
"se" => "TRANS SVENSKA",
"es" => "Hosting + registro o transferencia de nombre de dominio",
"pt" => "Alojamento + registo de dominio ou tranferencia"
);

$txt_dtcrm_hosting_only = array(
"fr" => "Hébergement seulement",
"en" => "Hosting only",
"hu" => "Csak hoszting",
"it" => "Solo hosting",
"nl" => "Alleen hosting",
"ru" => "TRANS Hosting only",
"de" => "TRANS Hosting only",
"zh" => "½ö¿Õ¼ä",
"pl" => "Tylko hosting",
"se" => "TRANS SVENSKA",
"es" => "S&oacute;lo hosting",
"pt" => "Apenas alojamento"
);

$txt_dtcrm_enter_domain_to_add  = array(
"fr" => "Veuillez entrer le nom de domaine que vous souhaitez ajouter:",
"en" => "Please enter the domain name you wish to add:",
"hu" => "Adja meg a hozzáadni kívánt domain nevet:",
"it" => "Per favore inserisci il nome a dominio che vorresti 
aggiungere:",
"nl" => "Vul de toe te voegen domeinnaam in:",
"ru" => "TRANS Please enter the domain name you wish to add:",
"de" => "TRANS Please enter the domain name you wish to add:",
"zh" => "ÇëÊäÈëÄúÏëÌí¼ÓµÄÓòÃû£º",
"pl" => "Proszê podaæ nazwê domeny do dodania:",
"se" => "TRANS SVENSKA",
"es" => "Por favor introduzca el nombre de dominio que desea a&ntilde;adir:",
"pt" => "Por favor indiqye o dominio que quer adicionar :"
);



$txt_dtcrm_your_domain_will_be_soon = array(
"fr" => "Votre domaine sera bientot validé:",
"en" => "Your domain name will be soon validated:",
"hu" => "A megadott domain hamarosan érvényesítésre kerül:",
"it" => "Il tuo nome a dominio verrà presto validato",
"nl" => "je ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúµÄÓòÃûºÜ¿ì¾ÍÄÜÉúÐ§£º",
"pl" => "Twoja domena zostanie pó¼niej sprawdzona ",
"se" => "TRANS SVENSKA",
"es" => "Su nombre de dominio ser&aacute; validado en breve",
"pt" => "O seu dominio será brevemente validado"
);

$txt_dtcrm_soon_an_admin_will_have_a_look = array(
"fr" => "Bientot un administrateur validra votre requette d'ajout
de nom de domaine sur votre compte. Vous n'avez pas les droits nécessaire
pour l'ajout de nom de domaine automatique. Pour obtenir l'ajout automatique
sans validation, ou sans enregistrement de nom de domaine veuillez contacter :",
"en" => "An administrator will examine your request shortly,
and validate the addition of this domain name to your account.
You curently don't have enough privileges to add domain names.
If you often add domain names, you can ask the administrator to
grant you the privilege of automatic domain name addition.
To request hosting for a new domain name, without administrator validation or domain name
registration, please write to:",
"hu" => "Hamarosan az adminisztrátor ellenõrzi és jóváhagyja ezt a domain
nevet. Önnek jelenleg nincs joga új domain neveket hozzáadni.
Ha gyakran visz fel új domain neveket, akkor megkérheti az adminisztrátort,
hogy adjon önnek ilyen jogok. Új domain név létrehozásához regisztráció nélkül
kérjük írjon erre címre:",
"it" => "Presto un amministratore controllerà la tua richiesta e 
accetterà l'aggiunta del dominio al tuo account. Attualmente non hai 
il privilegio di aggiungere nomi a dominio. Se aggiungi spesso nomi a 
dominio, puoi chiedere all'amministratore di darti il permesso di 
aggiungere domini in automatico. Per poter aggiungere o registrare 
domini senza la validazione da parte dell'amministratore, scrivi a:",
"nl" => "Een admin zal jouw aanvraag spoedig verwerken en de account valideren. Je hebt op dit moment geen toestemming om automatisch domeinnamen toe te voegen. Wanneer je vaak domeinnamen toevoeget kan je een Administrator vragen voor het privelege om automatisch domeinnamen toe te voegen. Om hosting voor een nieuw domein aan te vragen zonder admin validatie, stuur een bericht naar: ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ºÜ¿ì£¬¹ÜÀíÔ±»áºËÊµÄúµÄÐÅÏ¢¡£Èç¹ûÄúµÄÐÅÏ¢×¼È·ÎÞÎó£¬¹ÜÀíÔ±»á°ÑÄúÐÂÔöµÄÓòÃû¹ØÁªµ½ÄúµÄÕË»§¡£ÄúÔÝÊ±Ã»ÓÐÌí¼ÓÓòÃûµÄÈ¨ÏÞ¡£Èç¹ûÄúÐèÒª¾­³£Ìí¼ÓÓòÃû£¬Äú¿ÉÒÔÒªÇó¹ÜÀíÔ±ÊÚÓèÄú×Ô¶¯Ìí¼ÓÓòÃûµÄÈ¨ÏÞ¡£Èç¹ûÐèÒª£¬Çë·¢ÓÊ¼þµ½£º",
"pl" => "Pó¼niej administrator sprawdzi Twoje zapytanie oraz  poprawno¶æ dodawanej domeny do Twojego konta.
Aktualnie nie masz uprawnieñ do dodawania domen. Je¶li czêsto dodajesz domeny, mo¿esz zapytaæ administratora o mo¿liwo¶æ nadania uprawnieñ do automatycznego dodawania domen.
Aby mieæ mo¿liwo¶æ dodawania domen bez sprawdzania przez administratora proszê napisaæ do: ",
"se" => "TRANS SVENSKA",
"es" => "En breve un administrador atender&aacute; su petici&oacute;n y validar&aacute; el agregado de este 
dominio a su cuenta. De momento Ud. no tiene suficientes privilegios para a&ntilde;adir nombres de 
dominio autom&aacute;ticamente. Para poder a&ntilde;adir dominios no validados por el 
administrador o sin registrar debe escribir a::",
"pt" => "Assim que o administrador verifique este pedido e o valide 
este dominio será adicionado a sua conta.
Neste momentovocê  não tem permissões para adicionar dominios, poderá pedir para lhe atribuirem permissões para que  possa 
realizar esta operação automaticamente escrevendo para o:"
);

$txt_dtcrm_add_another_domain = array(
"fr" => "Vous pouvez ajouter un autre domaine:",
"en" => "You can add another domain name:",
"hu" => "Felvihet egy újabb domain nevet: ",
"it" => "Puoi aggiungere un altro dominio:",
"nl" => "Je kan een ander domein toevoegen: ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Äú¿ÉÒÔÔÙÌí¼ÓÒ»¸öÓòÃû£º",
"pl" => "Mo¿esz dodaæ kolejn± domenê:  ",
"se" => "TRANS SVENSKA",
"es" => "Puede a&ntilde;adir otro nombre de dominio",
"pt" => "Você poderá adicionar outro dominio:"
);

$txt_dtcrm_transfer_existing_or_new = array(
"fr" => "Voulez vous transférer un domaine existant ou en enregistrer un nouveau ?",
"en" => "Do you want to transfer an existing domain or register a new domain?",
"hu" => "Szeretne áttranszferálni egy létezõ domaint vagy újat szeretne regisztrálni ?",
"it" => "Vuoi trasferire un dominio esistente o registrare un nuovo 
dominio?",
"nl" => "Wil je een reeds geregistreerde domeinnaame transferen of een nieuwe registreren? ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúÒª¹ý»§Ò»¸öÏÖ´æµÄÓòÃû»¹ÊÇ×¢²áÒ»¸öÐÂµÄ£¿",
"pl" => "Czy chcesz przetransferowaæ istniej±c± domenê lub zarejestrowaæ now±? ",
"se" => "TRANS SVENSKA",
"es" => "¿Desea transferir un dominio existente o registrar uno nuevo?",
"pt" => "Quer tranferir um dominio existente ou registar um novo?"
);

$txt_dtcrm_register_new_domain = array(
"fr" => "Enregistrer un nouveau domaine",
"en" => "Register a new domain",
"hu" => "Új domain regisztriációja",
"it" => "Registrare un nuovo dominio",
"nl" => "Registreer een nieuwe domeinnaam ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "×¢²áÒ»¸öÐÂµÄÓòÃû",
"pl" => "Rejestruj now± domenê ",
"se" => "TRANS SVENSKA",
"es" => "Registrar un nuevo dominio",
"pt" => "Registar um novo dominio"
);

$txt_dtcrm_transfer_from_another_registrar = array(
"fr" => "Transferer un domaine existant depuis un autre registrar",
"en" => "Transfer an existing domain from another registrar",
"hu" => "Áttranszferál egy létezõ domaint egy másik regisztrátortól",
"it" => "Trasferire un dominio esistente da un altro registrar",
"nl" => "Transfer een domeinnaam van een andere registrar ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "´ÓÓòÃûËùÓÐÕßÄÇÀï¹ý»§Ò»¸öÏÖ´æµÄÓòÃû",
"pl" => "Transferuj istniej±c± domenê od innego rejestratora ",
"se" => "TRANS SVENSKA",
"es" => "Transferir un dominio existente desde otro registrador",
"pt" => "Tranferir um dominio existente de outro registar"
);

$txt_dtcrm_title_register_a_domain_name = array(
"fr" => "Enregistrer un nom de domaine",
"en" => "Register a domain name",
"hu" => "Domain nevet regisztrál",
"it" => "Registra un nome a dominio",
"nl" => "Registreer een domeinnaam ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "×¢²áÒ»¸öÓòÃû",
"pl" => "Rejestruj domenê",
"es" => "Registrar un nombre de dominio",
"pt" => "Registar um dominio"
);

$txt_dtcrm_register_step1 = array(
"fr" => "Etape 1 : vérifier la disponibilitée",
"en" => "Step 1: Verify availability",
"hu" => "Elsõ lépés: Elérhetõség ellenõrzése",
"it" => "Passo 1: verifica la disponibilità",
"nl" => "Stap 1: verifieer beschikbaarheid",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "²½Öè1£º¼ìÑéÓÐÐ§ÐÔ",
"pl" => "Krok 1: Weryfikacja dostêpno¶ci",
"se" => "TRANS SVENSKA",
"es" => "Paso 1: Verificar disponibilidad",
"pt" => "Passo 1 : Verificar a disponibilidade"
);

$txt_dtcrm_step2_enter_whois_info = array(
"fr" => "Etape 2 : entrez les informations du whois",
"en" => "Step 2: Enter whois information",
"hu" => "Második lépés: A whois információk megadása",
"it" => "Passo 2: Inserisci le informazioni per il whois",
"nl" => "Vul whois informatie in ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "²½Öè2£ºÊäÈëwhoisÐÅÏ¢",
"pl" => "Krok 2:Wprowadzanie danych o rejestruj±cym ",
"se" => "TRANS SVENSKA",
"es" => "Paso 2: Introducir informaci&oacute;n WHOIS",
"pt" => "Passo 2: Informação pra Whois"
);

$txt_crm_enter_the_domain_name_you_wish_to_register = array(
"fr" => "Entrer le nom de domaine que vous souhaitez enregistrer :",
"en" => "Enter the domain name you want to register:",
"hu" => "Adja meg a regisztrálni kívánt domain nevét:",
"it" => "Inserisci il nome a dominio che vuoi registrare:",
"nl" => "Vul domeinnaam in die je wilt registreren. ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÊäÈëÄúÒª×¢²áµÄÓòÃû",
"pl" => "Wprowad¼ nazwê domeny, ktor± chcesz zarejestrowaæ ",
"se" => "TRANS SVENSKA",
"es" => "Indique el nombre de dominio que desea registrar :",
"pt" => "Indique o dominio que quer registar :"
);

$txt_dtcrm_not_enough_privileges = array(
"fr" => "Vous n'avez pas les droits nécessaire a l'ajout de nom
de domaine sur votre compte. Si vous désirez ajouter des domaines
sans enregistrement, vous pouvez demander a l'administrateur de
le faire pour vous en écrivant à:",
"en" => "You curently don't have enough privileges to
add domain names. If you often add domain names, you can ask the
administrator to do so. To request hosting for a new domain without domain
name registration, please write to:",
"hu" => "Önnek jelenleg nincs joga új domain neveket hozzáadni.
Ha gyakran visz fel új domain neveket, akkor megkérheti az adminisztrátort,
hogy adjon önnek ilyen jogok. Új domain név létrehozásához regisztráció nélkül
kérjük írjon erre címre:",
"it" => "Attualmente non hai abbastanza privilegi per poter aggiungere 
nomi a dominio. Se ti capita di aggiungere o registrare spesso nomi a 
dominio devi chiedere all'amministratore. Per avere un nuovo nome a 
dominio da hostare senza la registrazione del nome, scrivi a: ",
"nl" => "Je hebt op dit moment geen toestemming om automatisch domeinnamen toe te voegen. Wanneer je vaak domeinnamen toevoeget kan je een Administrator vragen voor het privelege om automatisch domeinnamen toe te voegen. Om hosting voor een nieuw domein aan te vragen zonder admin validatie, stuur een bericht naar:  ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúÔÝÊ±Ã»ÓÐÌí¼ÓÓòÃûµÄÈ¨ÏÞ¡£Èç¹ûÄúÐèÒª¾­³£Ìí¼ÓÓòÃû£¬Äú¿ÉÒÔÒªÇó¹ÜÀíÔ±ÊÚÓèÄú×Ô¶¯Ìí¼ÓÓòÃûµÄÈ¨ÏÞ¡£Èç¹ûÐèÒª£¬Çë·¢ÓÊ¼þµ½£º",
"pl" => "Obecnie nie masz uprawnieñ do dodawania domen. Je¶li czêsto dodajesz domeny, mo¿esz zapytaæ administratora jak to zrobiæ.
Aby mieæ mo¿liwo¶æ dodawania domen do hosta bez rejestracji domen, proszê napisaæ do: ",
"se" => "TRANS SVENSKA",
"es" => "De momento Ud. no tiene suficientes privilegios para a&ntilde;adir nombres de dominio.
Si desea alojar un nuevo nombre de dominio sin registrarlo puede pedirle al administrador que
lo haga escribiendo a:",
"pt" => "Neste momento você  não tem permissões para adicionar dominios, poderá pedir para lhe atribuirem permissões para que  possa 
realizar esta operação automaticamente escrevendo para :"
);


$txt_dtcrm_not_correct_format = array(
"fr" => "Le nom de domaine que vous avez entré n'est pas valide. Veuillez essayer a nouveau.",
"en" => "Domain name is not in correct format. Please enter another name.",
"hu" => "A domain név nem megfelelõ formátumú. Kérjük válassszon másik nevet.",
"it" => "In nome a dominio non è nel formato corretto, perfavore 
scegli un altro nome.",
"nl" => "Dommeinnaam is niet in correct formaat. Vul AUB een andere naam in. ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúÊäÈëµÄÓòÃû²»ÕýÈ·£¬ÇëÖØÐÂÊäÈë",
"pl" => "Nazwa domeny ma nieprawid³owy format, proszê wprowadziæ inn± nazwê ",
"se" => "TRANS SVENSKA",
"es" => "El formato del nombre de dominio introducido es incorrecto. Por favor elija uno diferente",
"pt" => "o nome do dominio é inválido, verifique sff."
);

$txt_domain_name_already_hosted_here = array(
"fr" => "Ce nom de domaine est déjà hébergé ici. Veuillez en choisir un autre.",
"en" => "This domain name already hosted here, please enter another name.",
"hu" => "Ezt a domain-t már hosztoljuk, kérjük válasszon másik nevet.",
"it" => "Questo nome a dominio è già hostato qui, per favore scegli un 
altro nome. ",
"nl" => "Dit domein wordt reeds hier gehost. Vul AUB een andere naam in. ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Õâ¸öÓòÃûÒÑ±»Ê¹ÓÃ£¬ÇëÖØÐÂÊäÈëÒ»¸ö",
"pl" => "Hostujemy ju¿ t± domenê , proszê wybraæ inn± nazwê. ",
"se" => "TRANS SVENSKA",
"es" => "Este dominio ya se aloja en este servidor. Por favor seleccione otro nombre",
"pt" => "Este dominio ja se encontra alojado neste servidor, por favor escolha outro"
);

$txt_dtcrm_domain_name_now_ready = array(
"fr" => "Votre nom de domaine est pret :",
"en" => "Your domain name is now ready:",
"hu" => "Az ön domain neve már készen áll:",
"it" => "Il tuo nome a dominio è ora pronto:",
"nl" => "Jouw domeinnaam is nu gereed:",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúµÄÓòÃûÒÑÉèÖÃÍê±Ï£º",
"pl" => "Twoja domena jest gotowa do u¿ycia:",
"se" => "TRANS SVENSKA",
"es" => "Su nombre de dominio ya est&aacute; disponible",
"pt" => "O seu dominio está configurado"
);

$txt_dtcrm_you_can_check_config = array(
"fr" => "Vous pouvez maintenant utiliser votre domaine
en cliquant ici :",
"en" => "Now you can go to check its configuration by clicking here:",
"hu" => "Most ellenõrizheti a konfigurációját, ha ide kattint:",
"it" => "Ora puoi controllare la sua configurazione cliccando qui:",
"nl" => "Je kan nu de configuratie nakijken door hier te klikken ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "µã»÷ÕâÀï£¬Äú¿ÉÒÔ¼ì²éÓòÃûµÄÅäÖÃ£º",
"pl" => "Teraz mo¿esz przej¶æ do sprawdzenia ustawieñ klikaj±c tu: ",
"se" => "TRANS SVENSKA",
"es" => "Ahora puede comprobar su configuraci&oacute;n haciendo click aqu&iacute;",
"pt" => "Poderá agora verificar a sua configuração clicando aqui :"
);

$txt_dtcrm_or_you_can_add_another_domain = array(
"fr" => "Vous pouvez aussi ajouter un autre domaine:",
"en" => "Alternatively, you can add another domain name:",
"hu" => "Vagy hozzáadhat további domain neveket:",
"it" => "Oppure puoi aggiungere un altro dominio:",
"nl" => "Of je kan een nieuwe domeinnaam toevoegen ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "»òÕßÄú¿ÉÒÔÌí¼ÓÁíÍâÒ»¸öÓòÃû£º",
"pl" => "Mo¿esz dodaæ kolejn± domenê ",
"se" => "TRANS SVENSKA",
"es" => "O puede a&ntilde;adir otro nombre de dominio",
"pt" => "Ou poderá agora configurar outro dominio :"
);

$txt_dtcrm_could_not_connect_to_api = array(
"fr" => "Impossible de se connecter a l'API d'enregistrement de nom de domaine : veuillez essayez ultérieurement !",
"en" => "Could not connect to domain registration server: please try again later!",
"hu" => "Nem tudok csatlakozni a domain regisztrációs szerverhez: kérem próbálja késõbb",
"it" => "Non posso connettermi al server per la registrazione dei 
domini: per favore riprova più tardi",
"nl" => "Kon geen verbinding maken met de domein registratie server: probeer aub later!",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÎÞ·¨Á¬½Óµ½ÓòÃû×¢²á·þÎñÆ÷£ºÇëÉÔºòÔÙÊÔ£¡",
"pl" => "Nie mo¿na po³±czyæ siê z globalnym rejestatorem",
"se" => "TRANS SVENSKA",
"es" => "No se ha podido conectar al servidor de registro de nombres. Int&eacute;ntelo m&aacute;s tarde",
"pt" => "Não foi possivel contactar o servidor de registos, por favor tente mais tarde! "
);

$txt_sorry_domain_name_not_available1 = array(
"fr" => "Désolé, le nom de domaine ",
"en" => "Sorry, the domain name ",
"hu" => "Sajnálom, a domain név ",
"it" => "Mi dispiace, il nome a dominio ",
"nl" => "Sorry, de domeinnaam ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "¶Ô²»Æð,ÓòÃû",
"pl" => "Przepraszamy, domena jest niedostêpna",
"se" => "TRANS SVENSKA",
"es" => "Lamentablemente el nombre de dominio",
"pt" => "Desculpe, o dominio "
);

$txt_sorry_domain_name_not_available2 = array(
"fr" => "n'est PAS disponible, le serveur a retourner : ",
"en" => "is NOT available for registration. The registration server returned: ",
"hu" => "Nem regisztrálható. A regisztrációs szerver a következõvel tért vissza: ",
"it" => "NON è disponibile per la registrazione. Il server di 
registrazione dice: ",
"nl" => "is niet beschikbaar voor registratie. De registratie server antwoordde: ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÎÞ·¨×¢²á£¬ÓòÃû×¢²á·þÎñÆ÷·µ»ØÐÅÏ¢£º",
"pl" => "do rejestracji, Serwer rejestruj±cy zwróci³: ",
"se" => "TRANS SVENSKA",
"es" => "no est&aacute; disponible para registrar. El servidor de registros devolvió",
"pt" => "não esta disponivel para o registo. O servidor de registos returnou :"
);

$txt_please_select_the_3_contact_handles = array(
"fr" => "Veuillez selectionner les 3 contacts responsable que vous
voulez utiliser pour l'enregistrement du nom de domaine.",
"en" => "Please select the 3 contact handles you want to use for registering that
domain name.",
"hu" => "Kérem válasszon 3 kapcsolattartót, akiket az adott domain regisztrációjakor fel kíván tüntetni ",
"it" => "Per favore seleziona i tre contatti che vuoi usare per 
registrare il nome a dominio.",
"nl" => "Selecteer 3 contacts voor registratie van de domeinnaam  ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÇëÑ¡ÔñÈý¸öÄúÏ£ÍûÓÃÀ´×¢²áÕâ¸öÓòÃûµÄÁªÏµÈË¡£",
"pl" => "Proszê wybraæ trzy podrêczne kontakty, których chesz u¿ywaæ do rejestracji domen.",
"se" => "TRANS SVENSKA",
"es" => "Por favor seleccione 3 contactos responsables que quiera usar para registrar el nombre de dominio",
"pt" => "Por favor selecione 3 contactos para usar no registo desse dominio."
);

$txt_dtcrm_select_how_long_you_want_to_register = array(
"fr" => "Veuillez selectionner la période d'enregistrement pour ce domaine:",
"en" => "Select how long you want to register this domain name:",
"hu" => "Válassza ki mennyi idõre kívánja regisztrálni a domain nevet:",
"it" => "Scegli per quanto tempo vuoi registrare il dominio:",
"nl" => "Selecteer periode van registratie ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÇëÑ¡ÔñÕâ¸öÓòÃûµÄ×¢²áÆÚÏÞ£º",
"pl" => "Wybierz na jak d³ugo chcesz zarejestrowaæ domenê: ",
"se" => "TRANS SVENSKA",
"es" => "Seleccione la duración del registro del dominio",
"pt" => "Selecione o tempo do registo :"
);

$txt_dtcrm_year = array(
"fr" => "an",
"en" => "year",
"hu" => "év ",
"it" => "anno",
"nl" => "jaar ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Äê",
"pl" => "Rok ",
"se" => "TRANS SVENSKA",
"es" => "a&ntilde;o",
"pt" => "ano"
);

$txt_dtcrm_years = array(
"fr" => "ans",
"en" => "years",
"hu" => "évek ",
"it" => "anni",
"nl" => "jaren ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Äê",
"pl" => "lata ",
"se" => "TRANS SVENSKA",
"es" => "a&ntilde;os",
"pt" => "anos"
);

$txt_dtcrm_you_dont_have_a_client_id = array(
"fr" => "Vous n'avez pas d'ID client. Veuillez nous contacter",
"en" => "You don't have a client ID. Please contact us.",
"hu" => "Önnek nincs ügyfélazonosítója. Keressen bennünket.",
"it" => "Tu non hai un client ID. Per favore contattaci.",
"nl" => "Je hebt geen client ID, neem AUB contact met ons op.",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúÃ»ÓÐ¿Í»§ID¡£ÇëÁªÏµÎÒÃÇ¡£",
"pl" => "Nie masz ID klienta. Prosimy o kontakt. ",
"se" => "TRANS SVENSKA",
"es" => "No tiene ID de cliente. Por favor contacte al administrador",
"pt" => "Você não tem nenhum ID de cliente. Por favor contacte nos"
);

$txt_dtcrm_register_domain_step3 = array (
"fr" => "Etape 3 : procéder a l'enregistrement",
"en" => "Step 3: Proceed to registration",
"hu" => "Harmadik lépés: Regisztráció indítása ",
"it" => "Passo 3: procedi alla registrazione",
"nl" => "Stap 3 : Ga door naar registratie ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "²½Öè3£º½øÈë×¢²á",
"pl" => "Krok 3: Proces rejestracji",
"se" => "TRANS SVENSKA",
"es" => "Paso 3: Proceder con el registro",
"pt" => "Passo 3: Proceder ao registo"
);

$txt_dtcrm_remaining_on_your_account = array (
"fr" => "Restant sur votre compte : ",
"en" => "Remaining on your account: ",
"hu" => "Az ön számlájának egyenlege:",
"it" => "Rimanendo nel tuo account: ",
"nl" => "Resterend op jouw account: ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúÕÊ»§µÄÓà¶î£º",
"pl" => "Pozostalo na twoim koncie: ",
"se" => "TRANS SVENSKA",
"es" => "Restante en su cuenta",
"pt" => "Restante na sua conta : "
);

$txt_dtcrm_total_price = array (
"fr" => "Prix total : ",
"en" => "Total price: ",
"hu" => "Teljes ár: ",
"it" => "Prezzo totale: ",
"nl" => "Prijs totaal : ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "×Ü¼Û£º",
"pl" => "Suma: ",
"se" => "TRANS SVENSKA",
"es" => "Precio total",
"pt" => "Preço total : "
);

$txt_dtcrm_you_currently_dont_have_enough_funds = array (
"fr" => "Vous n'avez pas assez d'argent sur votre compte. Vous
serez redirigez sur notre system de paiement. Veuillez cliquer
sur le boutou suivant pour payer.",
"en" => "You currently don't have enough funds on your account. You will be
redirected to our payment system. Please click on the button below
to pay.",
"hu" => "Önnek nincs elegendõ pénz a számláján. Ön átirányításra kerül a mi fizetési rendszerünkhöz.
Fizetéshez kattintson az alábbi gombra.",
"it" => "Attualmente non hai abbastanza fondi nel tuo account. Sarai 
reindirizzato al nostro sistema di pagamento. Per favore clicca nel 
bottone sotto per pagare.",
"nl" => " Je hebt momenteen niet genoeg saldo op je account. Je wordt nu doorverwezen naar ons betaalsysteem. Klik alsjeblieft op de button hieronder om te betalen.",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúµÄÕÊ»§Óà¶î²»×ã¡£Ò³Ãæ½«»áÌø×ªµ½¸¶·ÑÏµÍ³, Çëµã»÷ÏÂÃæµÄ°´Å¥½øÐÐÖ§¸¶¡£",
"pl" => "Aktualnie nie posiadasz ¶rodków na koncie by dokoñczyæ transakcjê. Zostaniesz przekierowany do systemu p³atno¶ci. ",
"se" => "TRANS SVENSKA",
"es" => "Actualmente no tiene suficientes fondos en su cuenta. Se lo redireccionar&aacute; al
sistema de pago. Pulse en el bot&oacute;n indicado para pagar.",
"pt" => "Neste momento não tem crédito suficiente, será redirecionado para o sistema de pagamentos.
Queira clica no botão seguinte para efectuar o pagamento." 
);

$txt_dtcrm_you_have_enough_funds_proceed = array (
"fr" => "Vous avez assez d'argent sur votre compte pour procéder à
l'enregistrement. Veuillez cliquer sur le bouton suivant pour
continuer.",
"en" => "You have enough funds on your account to proceed with registration. Press
the confirm button to proceed.",
"hu" => "Önnek megfelelõ mennyiségû pénz van a számláján a regisztrációhoz.
Kattintson a jóváhagyás gombra és a megrendelése elindul.",
"it" => "Hai abbastanza fondi nel tuo account per procedere con la 
registrazione. Premi il bottone di conferma per procedere.",
"nl" => "Je hebt voldoende saldo om verder te gaan met de registratie. Klik op de confirm button om verder te gaan. ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÄúµÄÕÊ»§ÓµÓÐ×ã¹»µÄÓà¶îÀ´½øÐÐ×¢²á£¬Çëµã»÷È·ÈÏ¼ü¼ÌÐø×¢²á¡£",
"pl" => "Aktualnie posiadasz wystarczaj±co funduszy na koncie by zakoñczyæ rejestracjê. Wci¶nij potwierdzenie rejestracji by zakoñczyæ. ",
"se" => "TRANS SVENSKA",
"es" => "Ud. tiene fondos suficientes en su cuenta para proceder con el registro. Pulse
el bot&oacute;n de confirmar para continuar",
"pt" => "Você tem credito suficente para efectuar o pagamento. Tem de confirmar o pedido clicando no botão de confirmação"
);

$txt_dtcrm_button_paiement_done_checkout = array (
"fr" => "Paiement effectué, je veux continuer",
"en" => "Payment complete. Proceed to checkout",
"hu" => "Sikeres fizetés. Ellenõrzöm.. ",
"it" => "Pagamento effettuato, lasciami continuare.",
"nl" => "Betaling afgerond. Ga verder naar de checkout ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "¸¶¿îÍê±Ï£¬ÕýÔÚÐ£Ñé",
"pl" => "Dokonano p³atno¶ci, Proszê sprawdziæ ",
"se" => "TRANS SVENSKA",
"es" => "Pago realizado. Continuar",
"pt" => "Pagamento efectuado, deixe me efectuar a ultima validação."
);

$txt_dtcrm_button_proceed_to_registration = array (
"fr" => "Procéder a l'enregistrement du nom de domaine",
"en" => "Proceed to name-registration",
"hu" => "Név regisztráció indítása ",
"it" => "Procedi alla registrazione del nome",
"nl" => "Ga verder naar de domeinnaam registratie ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "½øÈëÓòÃû×¢²á",
"pl" => "Domena w trakcie rejestracji",
"se" => "TRANS SVENSKA",
"es" => "Proceder al registro de dominio",
"pt" => "Proceder ao registo do dominio"
);

$txt_dtcrm_registration_failed = array (
"fr" => "L'enregistrement à échoué",
"en" => "Registration failed",
"hu" => "A regisztráció sikertelen",
"it" => "Registrazione fallita",
"nl" => "Registratie gefaald ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "×¢²áÊ§°Ü",
"pl" => "Rejestracja zakoñczy³a siê b³êdem.",
"se" => "TRANS SVENSKA",
"es" => "Registro fallido",
"pt" => "O registo falhou" 
);

$txt_dtcrm_registration_succesfull = array (
"fr" => "Enregistrement réussit",
"en" => "Registration successful",
"hu" => "A regisztráció sikerült",
"it" => "Registrazione avvenuta con successo",
"nl" => "registratie successvol ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "×¢²á³É¹¦",
"pl" => "Rejestracja zakonczy³a siê powodzeniem",
"se" => "TRANS SVENSKA",
"es" => "Registro exitoso",
"pt" => "O registo foi efectuado correctamente"
);

$txt_dtcrm_server_said = array (
"fr" => "Le serveur a répondu : ",
"en" => "Server said: ",
"hu" => "A szerver válasza: ",
"it" => "Il server ha risposto: ",
"nl" => "Server antwoordde : ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "·þÎñÆ÷·µ»ØÐÅÏ¢£º",
"pl" => "Serwer odpowiedzia³ ",
"se" => "TRANS SVENSKA",
"es" => "El servidor dijo: ",
"pt" => "O servidor retornou : "
);

$txt_dtcrm_succesfully_added_domain = array (
"fr" => "Nom de domaine ajouté a la base de donnée avec succes",
"en" => "Successfully added your domain name to the hosting database",
"hu" => "Sikeresen hozzáadtuk a domain nevet az adatbázishoz ",
"it" => "Il tuo nome a dominio è stato aggiunto con successo al 
database di hosting",
"nl" => "Domeinnaam succesvol toegevoegd aan database ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÒÑ³É¹¦½«ÄúµÄÓòÃûÌí¼Óµ½Êý¾Ý¿âÖÐ",
"pl" => "Dodano Twoj± domenê do bazy utrzymywanych na naszym serwerze",
"se" => "TRANS SVENSKA",
"es" => "Dominio a&ntilde;adido satisfactoriamente a la base de datos",
"pt" => "O seu dominio foi adicionado a base de dados de alojamentos"
);

$txt_dtcrm_click = array (
"fr" => "Cliquez ",
"en" => "Click",
"hu" => "Kattinson ",
"it" => "Clicca",
"nl" => "klik ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "µã»÷",
"pl" => "Kliknij",
"se" => "TRANS SVENSKA",
"es" => "Click",
"pt" => "Clique"
);

$txt_dtcrm_here = array (
"fr" => "ici",
"en" => "here",
"hu" => "ide ",
"it" => "qui",
"nl" => "hier ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÕâÀï",
"pl" => "tutaj ",
"se" => "TRANS SVENSKA",
"es" => "aqu&iacute;",
"pt" => "aqui"
);

$txt_dtcrm_to_refresh_the_menu_or_add_another_domain = array(
"fr" => "pour réafficher le menu ou ajouter un nouveau nom de domaine.",
"en" => "to refresh the menu or add another domain name.",
"hu" => "a menü frissítéshez vagy további domain név hozzáadáshoz",
"it" => "per aggiornare il menu o aggiungere un altro nome a dominio.",
"nl" => "Om het menu te versen of een andere domeinnaam toe te voegen.",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ë¢ÐÂÖ÷²Ëµ¥»òÕßÌí¼ÓÁíÍâÒ»¸öÓòÃû¡£",
"pl" => "aby od¶wie¿yæ menu albo dodaæ nastêpn± domenê. ",
"se" => "TRANS SVENSKA",
"es" => "Para actualizar el men&uacute; o a&ntilde;adir otro dominio.",
"pt" => "para fazer \"refresh\" ou para adicionar outro dominio."
);

$txt_dtcrm_enter_the_domain_name_to_transfer = array(
"fr" => "Veuillez entrer le domaine que vous souhaitez transférer :",
"en" => "Please enter the domain name you wish to transfer:",
"hu" => "Adja meg az áttranszferálandó domain nevét",
"it" => "Inserisci il nome a dominio che vorresti trasferire:",
"nl" => "Vul alsjeblieft de domeinnaam in die je wenst te transferen: ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÇëÊäÈëÄúÏ£Íû×ªÈÃµÄÓòÃû£º",
"pl" => "Proszê wprowadziæ nazwê domeny, któr± chcesz przetransferowaæ:",
"se" => "TRANS SVENSKA",
"es" => "Por favor ingrese el dominio a transferir",
"pt" => "Indique por favor o dominio a tranferir : "
);

$txt_dtcrm_transfer_check_failed = array(
"fr" => "ECHEC DU TRANSFERE : le serveur d'enregistrement n'a pas répondu avec succès.",
"en" => "TRANSFER CHECK FAILED: registry server didn't reply successfuly.",
"hu" => "A TRANSZFER ELLENÕRZÉSE SIKERTELEN: a regiszrációs szerver nem felelt.",
"it" => "CHECK TRASFERIMENTO FALLITO: il server di 
registrazione 
non risponde affermativamente.",
"nl" => "TRANSFER CHECK FAILED: registry server gaf geen successvolle reply ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ð£ÑéÊ§°Ü£ºÓòÃû×¢²á·þÎñÆ÷Ã»ÓÐ´ð¸´¡£",
"pl" => "TRANSFER NIEUDANY: serwer rejestruj±cy nie by³ w stanie odpowiedzieæ.",
"se" => "TRANS SVENSKA",
"es" => "COMPROBACI&Oacute;N DE TRANSFERENCIA FALLIDA: El servidor de registros no respondi&oacute; satisfactoriamente",
"pt" => "A validação de transferência FALHOU: O servidor não retornou os valores correctamente."
);

$txt_dtcrm_transfer_check_failed = array (
"fr" => "ECHEC DU TEST DE TRANSFERABILITE",
"en" => "TRANSFER CHECK FAILED",
"hu" => "A TRANSZFER ELLENÕRZÉSE SIKERTELEN",
"it" => "CHECK TRASFERIMENTO FALLITO",
"nl" => "TRANSFER CHECK FAILED ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ð£ÑéÊ§°Ü",
"pl" => "SPRAWDZENIE TRANSFERU NIEUDANE ",
"se" => "TRANS SVENSKA",
"es" => "COMPROBACI&Oacute;N DE TRANSFERENCIA FALLIDA",
"pt" => "A validação de transferência FALHOU"
);

$txt_dtcrm_transfer_check_successfull = array (
"fr" => "SUCCES DU TEST DE TRANSFERABILITE",
"en" => "TRANSFER CHECK SUCCESSFUL",
"hu" => "A TRANSZFER ELLENÕRZÉSE SIKERES",
"it" => "CHECK TRASFERIMENTO AVVENUTO CON SUCCESSO",
"nl" => "TRANSFER CHECK SUCCESSFUL ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ð£Ñé³É¹¦",
"pl" => "TRANSFER ZAKONCZONY POWODZENIEM ",
"se" => "TRANS SVENSKA",
"es" => "COMPROBACI&Oacute;N DE TRANSFERENCIA SATISFACTORIA",
"pt" => "A validação de transferência foi validada CORRECTAMENTE"
);

$txt_dtcrm_step2_select_contact_transfer = array(
"fr" => "Etape 2 : selectionner les contacts pour le transfère de domaine",
"en" => "Step 2: select contacts for domain transfer",
"hu" => "Második lépés: jelölje ki a kapcsolattartókat a domain transzferhez",
"it" => "Step 2: seleziona i contatti per il trasferimento dominio",
"nl" => "Stap 2: selecter contacten voor domeinnaam transfer ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "²½Öè2£ºÑ¡Ôñ×ªÈÃÓòÃûµÄÁªÏµ·½Ê½",
"pl" => "Krok 2: wybierz kontakt dla transferu domeny ",
"se" => "TRANS SVENSKA",
"es" => "Paso 2: Seleccionar contactos para la transferencia de dominio",
"pt" => "Passo 3: Selecione os contactos para a tranferencia do dominio" 
);

$txt_dtcrm_step3_proceed_transfer = array(
"fr" => "Etape 3 : procéder au transfère",
"en" => "Step 3: proceed transfer",
"hu" => "Harmadik lépés: transzfer indítása ",
"it" => "Passo 3: procedi col trasferimento ",
"nl" => "Stap 3 : ga verder naar transfer",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "²½Öè3£º½øÐÐ×ªÈÃ",
"pl" => "Krok 3: Uruchom transfer ",
"se" => "TRANS SVENSKA",
"es" => "Paso 3: proceder con la transferencia",
"pt" => "Passo 3: Efectuar a tranferência" 
);

$txt_dtcrm_transaction_failed_try_again = array(
"fr" => "La transaction à échoué, veuillez réessayer",
"en" => "The transaction failed, please try again!",
"hu" => "A tranzakció sikertelen, kérem próbálja újra!",
"it" => "Transazione fallita, per favore riprova!",
"nl" => "De transactie is gefaald, probeer opnieuw ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "´¦ÀíÊ§°Ü£¬ÇëÔÙÊÔÒ»´Î£¡",
"pl" => "Transakcja nieudana, proszê spróbowaæ ponownie!",
"se" => "TRANS SVENSKA",
"es" => "Transacci&oacute;n fallida. Por favor int&eacute;ntelo de nuevo",
"pt" => "A comunicação com o gateway falhou, tente mais tarde!"
);

$txt_dtcrm_your_account_has_been_refund = array(
"fr" => "Votre compte à été crédité!",
"en" => "Your account has been credited!",
"hu" => "A számlája visszafizetésre került!",
"it" => "Il pagamento è stato aggiunto al vostro conto",
"nl" => "Jouw account is gecrediteerd ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ç®¿îÒÑ´æÈëÄúµÄÕË»§£¡",
"pl" => "Twoje konto zosta³o zasilone!",
"se" => "TRANS SVENSKA",
"es" => "Su cuenta ha sido acreditada",
"pt" => "A sua conta foi apagada!"
);

$txt_dtcrm_proceed_to_transfer_button = array(
"fr" => "Procéder au transfère",
"en" => "Proceed to transfer",
"hu" => "Transzfer indítása",
"it" => "Procedi col trasferimento",
"nl" => "Ga verder naar transfer ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "½øÐÐ×ªÈÃ",
"pl" => "Kontynuacja transferu ",
"se" => "TRANS SVENSKA",
"es" => "Ejecutar la transferencia",
"pt" => "Efectuar a tranferência"
);

$txt_transfer_from_another_registrar = array(
"fr" => "Transfère depuis un autre registrar vers ce serveur :",
"en" => "Transfer from another registrar to this server:",
"hu" => "Transzfer másik regisztrátortól erre a szerverre: ",
"it" => "Trasferisci da un altro registar a questo server: ",
"nl" => "Transfer vanaf een andere registrar naar deze server :",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "´Ó±ðµÄ×¢²áÉÌÇ¨ÒÆµ½ÕâÌ¨·þÎñÆ÷£º",
"pl" => "Transferowanie domeny od innego rejestratora na nasz serwer:",
"se" => "TRANS SVENSKA",
"es" => "Transferir desde otro registrador a este servidor",
"pt" => "Tranferir de outro REGISTRAR para este servidor"
);

$txt_dtcrm_step1_check_domain_is_transferable = array(
"fr" => "Etape 1 : vérifier que le domaine est transférable",
"en" => "Step1: check if domain is transferable",
"hu" => "Elsõ lépés: Transzferálható e a domain",
"it" => "Step 1: verifica se il dominio è trasferibile",
"nl" => "Stap 1 check of getransfered kan worden ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "²½Öè1£º¼ì²éÓòÃûÊÇ·ñ¿ÉÒÔÇ¨ÒÆ",
"pl" => "Krok 1: Sprawdz czy domenê mo¿na przetransferowaæ",
"se" => "TRANS SVENSKA",
"es" => "Paso 1: Comprobar si el dominio es transferible",
"pt" => "Passo 1: Vereficar se o seu dominio pode ser tranferido"
);

$txt_dtcrm_create_new_handle = array(
"fr" => "Créer un nouveau contact",
"en" => "Create a new handle",
"hu" => "Új kezelõ létrehozása",
"it" => "Crea un nuovo contatto",
"nl" => "Creeer een nieuw contact",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "½¨Á¢Ò»¸öÐÂ¾ä±ú ",
"pl" => "Tworzenie nowego identyfikatora",
"se" => "TRANS SVENSKA",
"es" => "Crear un nuevo contacto",
"pt" => "Criar um novo contacto"
);

$txt_dtcrm_indicate_required_field = array(
"fr" => "indique un champ requis",
"en" => "marked fields are required",
"hu" => "a megjelölt mezõk kötelezõek",
"it" => "indica i campi richiesti",
"nl" => "Gemarkeerde velden zijn verplicht",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "´òÐÇºÅµÄÑ¡ÏîÎª±ØÌîÏî",
"pl" => "oznaczona pola s± wymagfane ",
"se" => "TRANS SVENSKA",
"es" => "Los campos marcados son obligatorios",
"pt" => "os campos marcados, são obrigatórios"
);

$txt_dtcrm_name_for_this_handle = array(
"fr" => "Nom pour ce contact : ",
"en" => "Name for this handle: ",
"hu" => "A kezelõ neve:",
"it" => "Nome per questo contatto: ",
"nl" => "Naam voor deze contact:",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "¾ä±úµÄÃû×Ö ",
"pl" => "Nazwa dla tego identyfikatora: ",
"se" => "TRANS SVENSKA",
"es" => "Nombre para este contacto",
"pt" => "Nome para este contacto"
);

$txt_dtcrm_company = array(
"fr" => "Société : ",
"en" => "Company: ",
"hu" => "Cégnév: ",
"it" => "Società: ",
"nl" => "Bedrijf ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "¹«Ë¾£º",
"pl" => "Firma: ",
"se" => "TRANS SVENSKA",
"es" => "Empresa: ",
"pt" => "Empresa : "
);

$txt_dtcrm_firstname = array(
"fr" => "Prénom : ",
"en" => "First name: ",
"hu" => "Keresztnév: ",
"it" => "Nome: ",
"nl" => "Voornaam ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ãû£º",
"pl" => "Imiê ",
"se" => "TRANS SVENSKA",
"es" => "Nombre: ",
"pt" => "Nome : "
);

$txt_dtcrm_lastname = array(
"fr" => "Nom : ",
"en" => "Last name: ",
"hu" => "Vezetéknév: ",
"it" => "Achternaam: ",
"nl" => "TRANS ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÐÕ£º",
"pl" => "Nazwisko: ",
"se" => "TRANS SVENSKA",
"es" => "Apellido",
"pt" => "Apelido : "
);

$txt_dtcrm_street_addr1 = array(
"fr" => "Adresse 1 : ",
"en" => "Street address 1: ",
"hu" => "Utca 1: ",
"it" => "Indirizzo 1: ",
"nl" => "Addres 1 ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "½ÖµÀµØÖ·1£º",
"pl" => "Ulica (1)",
"se" => "TRANS SVENSKA",
"es" => "Direcci&oacute;n",
"pt" => "Morada : "
);

$txt_dtcrm_street_addr2 = array(
"fr" => "Adresse 2 : ",
"en" => "Street address 2: ",
"hu" => "Utca 2:",
"it" => "Indirizzo 2: ",
"nl" => "Addres 2 ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "½ÖµÀµØÖ·2£º",
"pl" => "ulica cd.",
"se" => "gatan",
"es" => "Direcci&oacute;n 2",
"pt" => "Morada 2 :"
);

$txt_dtcrm_street_addr3 = array(
"fr" => "Adresse 3 : ",
"en" => "Street address 3: ",
"hu" => "Utca 3: ",
"it" => "Indirizzo 3: ",
"nl" => "Addres 3 ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "½ÖµÀµØÖ·3£º",
"pl" => "ulica cd. ",
"se" => "gatan",
"es" => "Direcci&oacute;n 3",
"pt" => "Morada 3 :"
);

$txt_dtcrm_optional = array(
"fr" => "facultatif",
"en" => "optional",
"hu" => "opció",
"it" => "opzionale",
"nl" => "optioneel ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "Ñ¡Ïî",
"pl" => "Opcjopnalnie",
"se" => "TRANS SVENSKA",
"es" => "opcional",
"pt" => "Opcional"
);

$txt_dtcrm_if_applicable = array(
"fr" => "ci nécessaire",
"en" => "if applicable",
"hu" => "ha alkalmazható",
"it" => "se necessario",
"nl" => "wanneer toepasbaar ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÊÇ·ñÊÊÓÃ",
"pl" => "je¶li poprawny",
"se" => "TRANS SVENSKA",
"es" => "si es aplicable",
"pt" => "se for aplicável"
);

$txt_dtcrm_state = array(
"fr" => "Etat : ",
"en" => "State: ",
"hu" => "Állam:",
"it" => "Provincia: ",
"nl" => "Provincie ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÖÝ»òÊ¡£º",
"pl" => "Wojewód¼two ",
"se" => "TRANS SVENSKA",
"es" => "Provincia: ",
"pt" => "Destrito"
);

$txt_dtcrm_city = array(
"fr" => "Ville : ",
"en" => "City: ",
"hu" => "Város: ",
"it" => "Città: ",
"nl" => "Plaats ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "³ÇÊÐ£º",
"pl" => "Miasto",
"se" => "TRANS SVENSKA",
"es" => "Ciudad",
"pt" => "Cidade : "
);

$txt_dtcrm_zipcode = array(
"fr" => "Code postal : ",
"en" => "Zipcode: ",
"hu" => "Irányítószám: ",
"it" => "CAP: ",
"nl" => "Postcode ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "ÓÊ±à£º",
"pl" => "Kod pocztowy",
"se" => "TRANS SVENSKA",
"es" => "C&oacute;digo postal",
"pt" => "Código Postal : "
);

$txt_dtcrm_country = array(
"fr" => "Pays : ",
"en" => "Country: ",
"hu" => "Ország: ",
"it" => "Nazione: ",
"nl" => "Land ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "¹ú¼Ò£º",
"pl" => "Pañstwo",
"se" => "TRANS SVENSKA",
"es" => "Pais: ",
"pt" => "País : "
);

$txt_dtcrm_phone_number = array(
"fr" => "Numéro de téléphone : ",
"en" => "Phone number: ",
"hu" => "Telefonszám: ",
"it" => "Telefono: ",
"nl" => "Telefoon nummer ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "µç»°ºÅÂë£º",
"pl" => "Nr telefonu",
"se" => "TRANS SVENSKA",
"es" => "Tel&eacute;fono",
"pt" => "Telefone : "
);

// Translators: please insert your phone country code number here !
$txt_dtcrm_phone_formating = array(
"fr" => "format +33.XXXXXXXXX",
"en" => "+1.XXXXXXXXXX format",
"hu" => "+36.XXXXXXXX formátumban",
"it" => "formato +39.XXXXXXXXX",
"nl" => "+31.xxxxxxxxx ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "+86.XXXXXX ¸ñÊ½",
"pl" => "w formacie +48.XXxxxxxxx",
"se" => " +46.XXxxxxx",
"es" => "en formato +(prefijo).XXXXXXXX ",
"pt" => "no formato +351.XXXXXXXXX "
);

$txt_dtcrm_fax_number = array (
"fr" => "Numéro de fax : ",
"en" => "Fax number: ",
"hu" => "Fax szám: ",
"it" => "Fax: ",
"nl" => "Fax: ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "´«Õæ£º",
"pl" => "+48.XXxxxxxxxx",
"se" => "+46.XXxxxxx",
"es" => "Fax: ",
"pt" => "FAX : "
);

$txt_dtcrm_email = array (
"fr" => "Adresse email : ",
"en" => "Email address: ",
"hu" => "Email cím: ",
"it" => "Indirizzo email: ",
"nl" => "Email addres ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "µç×ÓÐÅÏäµØÖ·£º",
"pl" => "adres e-mail",
"se" => "TRANS SVENSKA",
"es" => "Direcci&oacute;n de e-mail: ",
"pt" => "Correio electrónico " 
);

$txt_dtcrm_must_be_valid_email = array (
"fr" => "Cette addresse DOIT être valide",
"en" => "This MUST be a valid address",
"hu" => "Valós címnek KELL lennie",
"it" => "Questo DEVE essere un indirizzo valido",
"nl" => "Dit MOET een geldig addres zijn ",
"ru" => "TRANS ",
"de" => "TRANS ",
"zh" => "±ØÐèÌîÐ´ÓÐÐ§µØÖ·",
"pl" => "Adres musi byæ prawid³owy",
"se" => "TRANS SVENSKA",
"es" => "Debe ser una direcci&oacute;n de correo v&aacute;lida",
"pt" => "Terá de ser um endereço válido"
);
?>