<?php


$txt_lists_main_title_subunsub = array (
  "fr" => "Abonner/désabonner une adresse email",
  "en" => "Subscribe/Unsubscribe an email address",
  "hu" => "TRANS Subscribe/Unsubscribe an email address",
  "it" => "Iscrivi/Cancella un indirizzo email",
  "nl" => "Abonneer/Deabbonneer een email adres",
  "ru" => "Subscribe/Unsubscribe an email address",
  "de" => "e-Mail Adresse hinzufuegen/entfernen",
  "zh" => "ÓÊ¼ş¶©ÔÄ/ÍË¶©",
  "pl" => "Adres e-mail subskrybcji/wypisania",
  "es" => "Suscribir/desuscribir una direcci&oacute;n de correo",
  "pt" => "Subscribe/Unsubscribe an email address");


$txt_lists_main_title_spammode = array (
  "fr" => "Spam mode",
  "en" => "Spam mode",
  "hu" => "TRANS Spam mode",
  "it" => "TRANS Spam mode",
  "nl" => "Spam mode",
  "ru" => "TRANS Spam mode",
  "de" => "Spam Modus",
  "zh" => "À¬»øÓÊ¼ş´¦ÀíÄ£Ê½",
  "pl" => "TRANS Spam mode",
  "es" => "Modo Spam",
  "pt" => "TRANS Spam mode");

$txt_lists_title_list_name = array (
  "fr" => "Nom de la liste :",
  "en" => "Name of the list:",
  "hu" => "TRANS Name of the list:",
  "it" => "Nome della lista:",
  "nl" => "Naam van de lijst",
  "ru" => "TRANS Name of the list:",
  "de" => "Name der Mailinglist:",
  "zh" => "ÓÊ¼şÁĞ±íµÄÃû×Ö£º",
  "pl" => "Nazwa listy mailingowej/pocztowej:",
  "es" => "Nombre de la lista:",
  "pt" => "TRANS Name of the list:");

$txt_lists_title_list_name = array (
  "fr" => "Propriétaire :",
  "en" => "Owner:",
  "hu" => "TRANS Owner:",
  "it" => "Proprietario:",
  "nl" => "Eigenaar",
  "ru" => "TRANS Owner:",
  "de" => "Besitzer:",
  "zh" => "ËùÓĞÕß£º",
  "pl" => "W³a¶ciciel",
  "es" => "Propietario:",
  "pt" => "TRANS Owner:");

$txt_lists_title_closedlist = array (
  "fr" => "Liste fermée :",
  "en" => "Closed list:",
  "hu" => "TRANS Closed list:",
  "it" => "Lista chiusa:",
  "nl" => "Gesloten lijst",
  "ru" => "TRANS Closed list:",
  "de" => "Geschlossene Liste:",
  "zh" => "ÒÑ¹Ø±ÕµÄÓÊ¼şÁĞ±í£º",
  "pl" => "Zamkniete listy",
  "es" => "Lista cerrada:",
  "pt" => "TRANS Closed list:");

$txt_lists_title_moderated = array (
  "fr" => "Modérée :",
  "en" => "Moderated:",
  "hu" => "TRANS Moderated:",
  "it" => "Moderata:",
  "nl" => "Gemodereerd",
  "ru" => "TRANS Moderated:",
  "de" => "Moderiert:",
  "zh" => "ÊÊÖĞµÄ",
  "pl" => "Moderowana:",
  "es" => "Moderada:",
  "pt" => "TRANS Moderated:");

$txt_lists_title_moderators = array (
  "fr" => "Modérateurs :",
  "en" => "Moderators:",
  "hu" => "",
  "it" => "Moderatori:",
  "nl" => "Moderators",
  "ru" => "",
  "de" => "Moderatoren:",
  "zh" => "°ßÖñ",
  "pl" => "Moderatorzy:",
  "es" => "Moderadores:",
  "pt" => "");

$txt_lists_title_subonlypost = array (
  "fr" => "Post inscrits uniquement :",
  "en" => "Subscribers only post:",
  "hu" => "",
  "it" => "Post solo agli iscritti:",
  "nl" => "Post alleen voor geabboneerden",
  "ru" => "",
  "de" => "Nur Mitglieder dürfen senden:",
  "zh" => "½ö·¢ËÍ¸ø¶©ÔÄÕß£º",
  "pl" => "",
  "es" => "S&oacute;lo suscriptos pueden postear:",
  "pt" => "");

$txt_lists_title_notifysub = array (
  "fr" => "Avertir des inscriptions :",
  "en" => "Notify new subscribtions:",
  "hu" => "",
  "it" => "Notifica nuove iscrizioni:",
  "nl" => "Notificatie nieuwe geabboneerden",
  "ru" => "",
  "de" => "Bei neuen Mitgliedern benachrichtigen",
  "zh" => "ÓĞĞÂµÄ¶©ÔÄÊ±ÌáÊ¾£º",
  "pl" => "",
  "es" => "Notificar nuevas suscripciones:",
  "pt" => "");

$txt_lists_title_noarchive = array (
  "fr" => "Pas d'archives :",
  "en" => "No archives:",
  "hu" => "",
  "it" => "Nessun archivio:",
  "nl" => "Geen archieven",
  "ru" => "",
  "de" => "Keine Archive anlegen:",
  "zh" => "²»´æµµ£º",
  "pl" => "",
  "es" => "No archivar:",
  "pt" => "");

$txt_lists_title_nosubconfirm = array (
  "fr" => "Pas de confirmation d'inscription :",
  "en" => "No subscribtion confirmation:",
  "hu" => "",
  "it" => "Nessuna conferma iscrizione:",
  "nl" => "Geen confirmatie abonnering",
  "ru" => "",
  "de" => "Keine Bestätigung der Mitgliedschaft:",
  "zh" => "Ã»ÓĞĞèÒªÈ·ÈÏµÄ¶©ÔÄ£º",
  "pl" => "",
  "es" => "Sin confirmaci&oacute;n de suscripci&oacute;n:",
  "pt" => "");

$txt_lists_title_noget = array (
  "fr" => "Pas de fonction get-N :",
  "en" => "No get-N function:",
  "hu" => "",
  "it" => "Disabilita funzione get-N:",
  "nl" => "Schakel get-N uit:",
  "ru" => "",
  "de" => "Keine get-N Funktion",
  "zh" => "²»Ê¹ÓÃget-N¹¦ÄÜ£º",
  "pl" => "",
  "es" => "Deshabilitar funci&oacute;n get-N: ",
  "pt" => "");

$txt_lists_title_subonlyget = array (
  "fr" => "Fonction get-N uniquement pour les inscrits :",
  "en" => "get-N function only for subscribers:",
  "hu" => "",
  "it" => "Funzione get-N solo iscritti:",
  "nl" => "Functie get-N alleen voor geabboneerden:",
  "ru" => "",
  "de" => "Get-N Funktion nur für Mitglieder",
  "zh" => "½ö¶©ÔÄÓÃ»§¿ÉÊ¹ÓÃget-N¹¦ÄÜ£º",
  "pl" => "",
  "es" => "Funci&oacute;n get-N s&oacute;lo para suscriptos:",
  "pt" => "");

$txt_lists_title_tocc = array (
  "fr" => "To: ou Cc: non obligatoire:",
  "en" => "To: or Cc: not mandatory:",
  "hu" => "",
  "it" => "To: o Cc: non obbligatori:",
  "nl" => "To: of Cc: niet verplicht:",
  "ru" => "",
  "de" => "To: oder Cc: nicht obligatorisch:",
  "zh" => "²»Ç¿ÖÆÌîĞ´ÊÕ¼şÈË»òÕß³­ËÍ¶ÔÏó£º",
  "pl" => "",
  "es" => "To: o Cc: no obligatorio:",
  "pt" => "");

$txt_lists_title_addtohdr = array (
  "fr" => "Ajouter entête To: :",
  "en" => "Add To: header:",
  "hu" => "",
  "it" => "Aggiungi all\'header To:",
  "nl" => "voeg toe: header:",
  "ru" => "",
  "de" => "Den To:-Header hinzufügen",
  "zh" => "Ìí¼Óµ½£ºÒ³Í·£º",
  "pl" => "",
  "es" => "Agregar cabecera To:",
  "pt" => "");

$txt_lists_title_notoccdenymails = array (
  "fr" => "Rejetter si sans To: ou Cc: :",
  "en" => "Deny if no To: or Cc::",
  "hu" => "",
  "it" => "Respingi se senza To: o Cc:",
  "nl" => "Weiger wanneer geen To: of Cc::",
  "ru" => "",
  "de" => "Mailing zurückweisen wenn kein To: oder Cc: vorhanden:",
  "zh" => "Èç¹ûÃ»ÓĞÌîĞ´ÊÕ¼şÈË»òÕß³­ËÍ¶ÔÏóÔò²»·¢ËÍ£º",
  "pl" => "",
  "es" => "Rechazar si falta To: o Cc:",
  "pt" => "");

$txt_lists_title_noaccessdenymails = array (
  "fr" => "Alerte lorsque post et pas d'acces :",
  "en" => "Notify when post and no access:",
  "hu" => "",
  "it" => "Notifica quando invia e nessun accesso:",
  "nl" => "Notificatie bij mailing en geen toegang",
  "ru" => "",
  "de" => "Benachrichtigung bei Mailing und kein Zugang: ",
  "zh" => "·¢ËÍºóÌáÊ¾£º",
  "pl" => "",
  "es" => "Notificar post enviados sin acceso:",
  "pt" => "");

$txt_lists_title_nosubonlydenymails = array (
  "fr" => "Alerte lorsque post et pas inscrit :",
  "en" => "Notify when post and not subscribed:",
  "hu" => "",
  "it" => "Notifica quando invia e non iscritto:",
  "nl" => "Notificatie bij mailing en geen toegang",
  "ru" => "",
  "de" => "Benachrichtigung bei Mailing ohne Mitgliedschaft:",
  "zh" => "·¢ËÍºóÎ´¶©ÔÄÊ±ÌáÊ¾£º",
  "pl" => "",
  "es" => "Notificar post de no suscriptos:",
  "pt" => "");

$txt_lists_title_prefix = array (
  "fr" => "Préfix du sujet :",
  "en" => "Subject prefix:",
  "hu" => "",
  "it" => "Prefisso del subject:",
  "nl" => "Prefix onderwerp",
  "ru" => "",
  "de" => "Präfix für den Betreff:",
  "zh" => "Ö÷ÌâµÄÇ°×º",
  "pl" => "",
  "es" => "Prefijo del asunto:",
  "pt" => "");

$txt_lists_title_memorymailsize = array (
  "fr" => "Taille memoire max des messages :",
  "en" => "Max mail memory size:",
  "hu" => "",
  "it" => "Massima memoria dei messaggi:",
  "nl" => "Max Mail geheugen",
  "ru" => "",
  "de" => "Maximale Grösse des Nachrichtenspeichers:",
  "zh" => "ÓÊ¼şÏµÍ³¿ÉÊ¹ÓÃµÄ×î´óÄÚ´æ¿Õ¼ä£º",
  "pl" => "",
  "es" => "Memoria m&aacute;xima del mensaje:",
  "pt" => "");

$txt_lists_title_relayhost = array (
  "fr" => "Serveur SMTP relai :",
  "en" => "SMTP relay server:",
  "hu" => "",
  "it" => "Server relay SMTP:",
  "nl" => "SMTP Relay Server",
  "ru" => "",
  "de" => "SMTP Relay Server:",
  "zh" => "SMTP×ª·¢·şÎñÆ÷£º",
  "pl" => "",
  "es" => "Servidor de relay SMTP:",
  "pt" => "");

$txt_lists_title_digestinterval = array (
  "fr" => "Intervale du digest :",
  "en" => "Digest interval:",
  "hu" => "",
  "it" => "Intervallo di digest:",
  "nl" => "Digest interval",
  "ru" => "",
  "de" => "Intervall der Zusammenfassungen:",
  "zh" => "ÕªÒªµÄÑ¡ÔñÇø¼ä",
  "pl" => "",
  "es" => "Intervalo de recopilaci&oacute;n:",
  "pt" => "");

$txt_lists_title_digestmaxmails = array (
  "fr" => "Nombre de message max du digest :",
  "en" => "Digest max mails:",
  "hu" => "",
  "it" => "Numero massimo emails di digest:",
  "nl" => "Digest Maximaal aantal berichten: ",
  "ru" => "",
  "de" => "Maximale Zahl der Nachrichten in der Zusammenfassung:",
  "zh" => "ÕªÒªÓÊ¼şµÄ×î´ó·¢ËÍÊıÁ¿£º",
  "pl" => "",
  "es" => "N&uacute;mero m&aacute;ximo de emails por recopilaci&oacute;n:",
  "pt" => "");

$txt_lists_title_verp = array (
  "fr" => "VERP :",
  "en" => "VERP:",
  "hu" => "",
  "it" => "VERP:",
  "nl" => "VERP:",
  "ru" => "",
  "de" => "VERP:",
  "zh" => "VERP£º",
  "pl" => "",
  "es" => "VERP",
  "pt" => "");

$txt_lists_title_maxverprecips = array (
  "fr" => "Nombre max de destinataire VERP :",
  "en" => "Max VERP recipients:",
  "hu" => "",
  "it" => "Numero massimo di destinatari VERP:",
  "nl" => "Max VERP ontvangers:",
  "ru" => "",
  "de" => "Maximale Empfänger des VERP:",
  "zh" => "×î´óVERPÊÕ¼şÈËÊıÁ¿£º",
  "pl" => "",
  "es" => "Max destinatarios VERP:",
  "pt" => "");

$txt_lists_title_delimiter = array (
  "fr" => "Délimiteur :",
  "en" => "Delimiter:",
  "hu" => "",
  "it" => "Delimiter:",
  "nl" => "Delimiter",
  "ru" => "",
  "de" => "Delimiter:",
  "zh" => "·Ö¸ô·û£º",
  "pl" => "",
  "es" => "Delimitar:",
  "pt" => "");

$txt_lists_title_owner = array (
  "fr" => "Propriétaire :",
  "en" => "Owner:",
  "hu" => "",
  "it" => "Proprietario:",
  "nl" => "Eigenaar:",
  "ru" => "",
  "de" => "Besitzer:",
  "zh" => "ËùÓĞÕß£º",
  "pl" => "",
  "es" => "Propietario:",
  "pt" => "");

$txt_lists_title_customheaders = array (
  "fr" => "Entetes personalisés :",
  "en" => "Custom headers:",
  "hu" => "",
  "it" => "Headers personalizzati:",
  "nl" => "Persoonlijke headers ",
  "ru" => "",
  "de" => "Benutzerdefinierter Header",
  "zh" => "×Ô¶¨ÒåµÄÒ³Í·£º",
  "pl" => "",
  "es" => "Cabeceras personalizadas:",
  "pt" => "");

$txt_lists_title_delheaders = array (
  "fr" => "Effacer les headers :",
  "en" => "Delete headers:",
  "hu" => "",
  "it" => "Cancella headers:",
  "nl" => "Verwijder headers",
  "ru" => "",
  "de" => "zu löschende Header:",
  "zh" => "É¾³ıÒ³Í·£º",
  "pl" => "",
  "es" => "Eliminar cabeceras:",
  "pt" => "");

$txt_lists_title_access = array (
  "fr" => "Liste d'accès :",
  "en" => "Access list:",
  "hu" => "",
  "it" => "Lista d\'accesso:",
  "nl" => "Toegangs lijst",
  "ru" => "",
  "de" => "Zugangsliste:",
  "zh" => "È¨ÏŞÁĞ±í£º",
  "pl" => "",
  "es" => "Lista de acceso:",
  "pt" => "");

$txt_lists_title_footer = array (
  "fr" => "Pied de message aditionnel :",
  "en" => "Added footer:",
  "hu" => "",
  "it" => "Aggiungi footer:",
  "nl" => "Toegevoegde footer",
  "ru" => "",
  "de" => "Benutzerdefinierter zusätzlicher Footer:",
  "zh" => "×·¼ÓÒ³½Å",
  "pl" => "",
  "es" => "Pie de mensaje adicional:",
  "pt" => "");


$txt_lists_title_spammode = array (
  "fr" => "Caché les adresse emails (spammode) :",
  "en" => "Hide the email addresses (spammode):",
  "hu" => "TRANS Hide the email addresses (spammode):",
  "it" => "TRANS Hide the email addresses (spammode):",
  "nl" => "Verberg email addressen (spammode):",
  "ru" => "TRANS Hide the email addresses (spammode):",
  "de" => "Email-Adressen verstecken (spammode):",
  "zh" => "Òş²ØÓÊ¼şµØÖ·(spammode)£º",
  "pl" => "TRANS Hide the email addresses (spammode):",
  "es" => "Ocultar direcciones de email (spammode):",
  "pt" => "TRANS Hide the email addresses (spammode):");

  
$txt_lists_title_webarchive = array (
  "fr" => "Activé l'archivage web :",
  "en" => "Enable webarchive:",
  "hu" => "",
  "it" => "Abilita archivio web:",
  "nl" => "Activeer webarchief",
  "ru" => "",
  "de" => "Webarchiv aktivieren",
  "zh" => "Ê¹ÓÃ¹éµµ",
  "pl" => "",
  "es" => "Habilitar archivo web:",
  "pt" => "");

$txt_lists_title_rcfile = array (
  "fr" => "Modèle personalisé :",
  "en" => "Own template:",
  "hu" => "",
  "it" => "Template personalizzato:",
  "nl" => "Persoonlijk template",
  "ru" => "",
  "de" => "Benutzerdefiniertes Template:",
  "zh" => "¸öÈËÄ£°å£º",
  "pl" => "",
  "es" => "Plantilla propia:",
  "pt" => "");
  
$txt_lists_title_recreatewa = array (
  "fr" => "Recréer :",
  "en" => "Recreate:",
  "hu" => "",
  "it" => "Ricrea:",
  "nl" => "Hercreer",
  "ru" => "",
  "de" => "Wiederherstellen:",
  "zh" => "ÖØĞÂ´´½¨£º",
  "pl" => "",
  "es" => "Recrear:",
  "pt" => "");

$txt_lists_title_deletewa = array (
  "fr" => "Effacer :",
  "en" => "Delete:",
  "hu" => "",
  "it" => "Cancella:",
  "nl" => "Verwijder:",
  "ru" => "",
  "de" => "Löschen:",
  "zh" => "É¾³ı£º",
  "pl" => "",
  "es" => "Eliminar:",
  "pt" => "");

$txt_lists_hlp_list_name = array (
  "fr" => "Nom de la liste.",
  "en" => "Name of the list.",
  "hu" => "",
  "it" => "Nome della lista.",
  "nl" => "Naam van de lijst",
  "ru" => "",
  "de" => "Name der Mailingliste:",
  "zh" => "ÓÊ¼şÁĞ±íµÄÃû³Æ£º",
  "pl" => "",
  "es" => "Nombre de la lista:",
  "pt" => "");

$txt_lists_hlp_listaddress = array (
  "fr" => "Ce champ contient toutes les adresses que MLMMJ voit comme adresse de liste (voire l\'option tocc). La première est l\'adresse principale.",
  "en" => "This field contains all addresses which mlmmj sees as listaddresses (see tocc option). The first one is the one used as the primary one, when mlmmj sends out mail.",
  "hu" => "",
  "it" => "Questo campo contiene gli indirizzi...",
  "nl" => "Dit veld bevat alle adressen die mlmmj als lijst adressen beschouwd (zie tocc optie). Het eerste adress is gebruikt als primaire wanneer mlmmj mail verstuurd.",
  "ru" => "",
  "de" => "Dieses Feld beinhaltet alle Adressen, die als Listadressen gesehen werden. Die erste ist die Primäradresse, die zum Versenden der Mails benutzt wird.",
  "zh" => "´Ë´¦°üº¬ÁËËùÓĞµÄÓÊ¼şµØÖ·£¬mlmmj½«ËüÊÓ×÷Ò»¸öµØÖ·ÁĞ±í£¨ÏêÇéÇë²Î¿¼tocc£©¡£µ±mlmmj·¢ËÍÓÊ¼şµÄÊ±ºò£¬µÚÒ»¸öµØÖ·±»ÊÓÎªÖ÷ÒªµØÖ·¡£",
  "pl" => "",
  "es" => "Este campo contiene todas las direcciones que MLMMJ ve como direcciones de lista (ver opci&oacute;n tocc). La primera es la usada como primaria cuando mlmmj env&iacute;a un mail.",
  "pt" => "");

$txt_lists_hlp_closedlist = array (
  "fr" => "La liste est-elle fermée. Si elle est fermée, les inscription et désinscription sont désactivées.",
  "en" => "Is the list is open or closed. If it\'s closed subscribtion and unsubscription via mail is disabled.",
  "hu" => "",
  "it" => "Se selezionato la lista è chiusa. Le iscrizioni e cancellazioni dalla lista sono disattivate.",
  "nl" => "Is de lijst open of gesloten Wanneer gesloten is abbonneren en deabbonneren via mail gedeactiveerd.",
  "ru" => "",
  "de" => "Offene oder geschlossene Liste. Falls die Liste geschlossen ist, ist keine An- oder Abmeldung per E-Mail m&ouml;glich",
  "zh" => "ÓÊ¼şÁĞ±íµÄ×´Ì¬ÊÇ¿ªÆô»¹ÊÇ¹Ø±Õ¡£Èç¹ûËü¹Ø±ÕÁË£¬ÓÊ¼ş¶©ÔÄ/ÍË¶©¹¦ÄÜ½«²»ÄÜÊ¹ÓÃ¡£",
  "pl" => "",
  "es" => "La lista est&aacute; cerrada. Las suscripciones y desuscripciones est&aacute;n desactivadas.",
  "pt" => "");

$txt_lists_hlp_moderated = array (
  "fr" => "Si ce drapeau est positionné, alors la liste est modérée",
  "en" => "If this flag is set, the email addresses in the field moderators will act as moderators for the list.",
  "hu" => "",
  "it" => "Se selezionato, la lista diventa moderata.",
  "nl" => "Wanneer aangevinkt, worden de email adressen in het veld moderators gezien als moderators voor de lijst.",
  "ru" => "",
  "de" => "Falls Haken gesetzt ist, wirken die Adressen die als Moderatoren gelistet sind als Moderatoren f&uuml;r die Liste.",
  "zh" => "Èç¹ûÉèÖÃÁËÕâ¸ö²ÎÊı£¬°ßÖñÎ»ÖÃÉÏÏÔÊ¾µÄÓÊ¼şµØÖ·½«±»ÊÓ×÷¸ÃÓÊ¼şÁĞ±íµÄ¹ÜÀíÔ±¡£",
  "pl" => "",
  "es" => "Si se activa esta casilla la lista est&aacute; moderada.",
  "pt" => "");

$txt_lists_hlp_moderators = array (
  "fr" => "Ceci est la liste des modérateurs.",
  "en" => "This is the list of moderators.",
  "hu" => "",
  "it" => "Lista dei moderatori (1 per linea).",
  "nl" => "Dit is de lijst van moderators",
  "ru" => "",
  "de" => "Dies ist die Liste der Moderatoren",
  "zh" => "°ßÖñÁĞ±í£º",
  "pl" => "",
  "es" => "Lista de moderadores (1 por linea).",
  "pt" => "");

$txt_lists_hlp_tocc = array (
  "fr" => "Si ce drapeau est positionné, la liste ne doit pas forcément être présente dans les champs To: ou Cc: du header du message.",
  "en" => "If this flag is set, the list address does not have to be in the To: or Cc: header of the email to the list (interesting for aliases addressing multiple lists).",
  "hu" => "",
  "it" => "",
  "nl" => "Wanneer aangevinkt, hoeft de lijst van adressen niet in te To: of Cc: header van de email lijst te staan ( voor aliassen voor meerdere lijsten)",
  "ru" => "",
  "de" => "Falls Haken gesetzt ist, muss die Listen-Adresse bei Mails an die Liste nicht im To: oder Cc: Feld stehen (interessant bei Aliasen die mehrere Listen umfassen).",
  "zh" => "Èç¹ûÉèÖÃÁËÕâ¸ö²ÎÊı£¬ÁĞ³öÀ´µÄÓÊ¼şµØÖ·½«²»±Ø±»ÌîÈëÊÕ¼şÈË»ò³­ËÍÀ¸ÖĞ£º",
  "pl" => "",
  "es" => "Si se activa esta opci&oacute;n la lista de direcciones no debe estar en las cabeceras To: o Cc: del mail a la lista.",
  "pt" => "");

$txt_lists_hlp_subonlypost = array (
  "fr" => "Quand ce drapeau est positionné, seuls les inscrits peuvent envoyer. La vérification est faite en fonction du &quot;From:&quot; du header.",
  "en" => "When this flag is set, only people who are subscribed to the list, are allowed to post to it. The check is made against the &quot;From:&quot; header.",
  "hu" => "",
  "it" => "Quando selezionato, solo gli iscritti potranno postare nella lista. La verifica viene effettuata nel campo &quot;From:&quot; dell\'header.",
  "nl" => "Wanneer aangevinkt, alleen mensen die geabboneerd zijn op de lijst hebben permissie om posts erin te maken. Deze check wordt gedaan door middel van de &quot;From:&quot; header.",
  "ru" => "",
  "de" => "Falls Haken gesetzt ist, d&uuml;rfen nur Leute die die Liste abonniert haben auf ihr ver&ouml;ffentlichen. Der Check wird gegen den &quot;From:&quot; durchgef&uuml;hrt.",
  "zh" => "Èç¹ûÉèÖÃÁËÕâ¸ö²ÎÊı£¬½ö½ö¶©ÔÄÁËÕâ¸öÓÊ¼şÁĞ±íµÄÈË²ÅÄÜÏòÕâ¸öÓÊ¼şÁĞ±í·¢ËÍÓÊ¼ş¡£",
  "pl" => "",
  "es" => "Si se activa esta casilla, s&oacute;lo los suscriptos a la lista pueden postear. Se verificar&aacute; la cabecera &quot;From:&quot;",
  "pt" => "");

$txt_lists_hlp_prefix = array (
  "fr" => "Préfixe de la ligne Sujet: (Subject:) des messages de la liste. Ceci va altéré la ligne Sujet: et ajouter un préfixe si il n\'est pas déjà présent.",
  "en" => "The prefix for the Subject: line of mails to the list. This will alter the Subject: line, and add a prefix if it\'s not present elsewhere.",
  "hu" => "",
  "it" => "",
  "nl" => "Prefix voor de onderwerp: regel voor berichten naar de lijst. Dit zal de Subject: regel veranderen en een prefix toevoegen als deze elders nog niet aanwezig is.",
  "ru" => "",
  "de" => "Das Pr&auml;fix f&uuml;r den Betreff der Mails von der Liste. Dies wird die Betreffzeile &auml;ndern und das Pr&auml;fix voranstellen, falls es nicht irgendwo anders vorhanden ist.",
  "zh" => "Ö÷ÌâµÄÇ°×º£ºÓÊ¼şÁĞ±íµÄ´óĞ¡¡£Õâ½«¸ü¸ÄÓÊ¼şµÄÖ÷Ìâ£¨Ôö¼ÓÒ»¸öĞĞÊı£©£¬Èç¹ûÔÚ±ğ´¦²»´æÔÚÕâ¸öÇ°×ºµÄ»°¡£",
  "pl" => "",
  "es" => "Prefijo de la linea &quot;Asunto:&quot; en los mails a la lista. Este prefijo se agregar&aacute; si no est&aacute; presente en otra parte.",
  "pt" => "");

$txt_lists_hlp_owner = array (
  "fr" => "Les adresses emails de ces champs (1 par ligne) recevront les messages pour liste-owner@nom-de-domaine.tld",
  "en" => "The email addresses in this fields (1 per line) will get mails to listname-owner@listdomain.tld",
  "hu" => "",
  "it" => "Questi indirizzi email (1 per linea) riceveranno i messaggi destinati a lista-owner@dominio.tld.",
  "nl" => "De email adressen in dit veld (1 per regel) zullen mail gestuurd naar listnaam-eigenaar@lijstdomein.tld ontvangen.",
  "ru" => "",
  "de" => "Die EMailadressen in diesen Feldern (Eine pro Zeile) werden die Mails an listname-owner@listdomain.tld erhalten.",
  "zh" => "ÔÚÕâ¸öÇøÓòÖĞµÄÓÊ¼şµØÖ·£¨Ã¿ĞĞÒ»¸ö£©½«ÊÕµ½·¢ËÍ¸ølistname-owner@listdomain.tldµÄÓÊ¼ş",
  "pl" => "",
  "es" => "Las direcciones de mail en este campo (1 por linea) recibir&aacute;n los mensajes para: propietario_de_la_lista@dominio.tld.",
  "pt" => "");

$txt_lists_hlp_customheaders = array (
  "fr" => "Ces champs contiennent les champs qui doivent être ajouté a chaques message. C\'est ici que vous pouvez ajouter le champs Reply-To: dans le header si cela est ce que vous voulez.",
  "en" => "These headers are added to every mail coming through. This is the place you want to add Reply-To: header in case you want such.",
  "hu" => "",
  "it" => "",
  "nl" => "Deze headers worden toegevoegd aan ieder bericht dat langskomt. Dit is de de plaats waar je Reply-To: wilt toevoegen in mocht je dat willen.",
  "ru" => "",
  "de" => "Diese Header werden an alle durchgehenden Mails angeh&auml;ngt. Hier w&uuml;rden Sie Reply-To: Header einf&uuml;gen, falls Sie diese ben&ouml;tigen.",
  "zh" => "ÕâĞ©Ò³Í·½«±»Ìí¼Óµ½Ã¿·â¾­¹ıµÄÓÊ¼şÖĞ£¬Èç¹ûÄãÏëÒªÕâÑùµÄ»°£¬±ÈÈçÌí¼ÓÒ»¸ö»Ø¸´µØÖ·£¬Õâ½«ÊÇ¸ö²»´íµÄÎ»ÖÃ¡£",
  "pl" => "",
  "es" => "Estas cabeceras son agregadas a todo mail entrante. Aqu&iacute; es donde puede agregar el encabezado &quot;Reply-To:&quot; si asi lo desea.",
  "pt" => "");

$txt_lists_hlp_delheaders = array (
  "fr" => "Dans ces champs est spécifié *UN* token du header par ligne. Si les champs sont comme il suit :<br><br>Received:<br>Message-ID:<br><br>Alors toutes les occurences de ces headers dans les messages entrants de la liste seront effacé. From: et Return-Path: sont effacé dans tous les cas.",
  "en" => "In those fields is specified *ONE* headertoken to match per line. If the fields are like this:<br><br>Received:<br>Message-ID:<br><br>Then all occurences of these headers in incoming list mail will be deleted. From: and Return-Path: are deleted no matter what.",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "In diesen Feldern k&ouml;nnen sie *EINEN* Header pro Zeile definieren, die eingehenden Mails entfernt werden sollen. Sieht das Ganze so aus:<br><br>Received:<br>Message-ID:<br><br>werden alle entsprechenden Header in eingehenden Mails entfernt. From: und Return-Path: werden so oder so entfernt.",
  "zh" => "ÔÚÕâĞ©ÇøÓòÖĞ±ê¼ÇÁËÒ»Ğ©ĞèÒª¹ıÂËµÄÒ³Í·ĞÅÏ¢¡£Èç¹ûÕâ¸öÇøÓòÖĞÌîĞ´µÄÊÇÕâÑùµÄÄÚÈİ£º<br><br>Received:<br>Message-ID:<br><br>ÄÇÃ´ËùÓĞµÄ°üº¬ÕâĞ©ÄÚÈİµÄÓÊ¼ş¶¼½«±»É¾³ı¡£From: ºÍ Return-Path:ÊÇÎŞÂÛÈçºÎ¶¼»á±»É¾³ıµÄ¡£",
  "pl" => "",
  "es" => "En este archivo se especifica *UN* control de cabecera por l&iacute;nea. Si el archivo consiste de:<br><br>Received:<br>Message-ID:<br><br>todas las apariciones de estos encabezados en el correo entrante de la lista ser&aacute;n borradas. From: y Return-Path: son borrados por defecto.",
  "pt" => "");

$txt_lists_hlp_access = array (
  "fr" => "Lire la partie README.access de la doc de MLMMJ.",
  "en" => "If this file exists, all headers of a post to the list is matched against the rules. The first rule to match wins. NOTE: the default action is to deny access (reject the mail) so take care if you write something here",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Wenn dieses File existiert, werden alle Header eines Posts gegen diese Regeln gechecked. Die erste Regel die zutrifft, gewinnt.<br><br>CAVE: Die Standardaktion ist es, die Mail abzulehnen. Also Vorsicht was Sie hier hereinschreiben.",
  "zh" => "Èç¹û´æÔÚÕâ¸öÎÄ¼ş£¬ËùÓĞ·¢ÍùÕâ¸öÓÊ¼şÁĞ±íµÄÓÊ¼şµÄÒ³Í·ĞÅÏ¢¶¼½«±»¹ıÂË¡£×îÇ°ÃæµÄ¹æÔòÓÅÏÈÉúĞ§¡£Çë×¢Òâ£ºÄ¬ÈÏµÄ¹æÔòÊÇ¾Ü¾øËùÓĞÓÊ¼ş£¬ËùÒÔ£¬µ±ÄãÔÚÕâ¸öÎÄ¼şÖĞĞ´ÏÂ¹æÔòµÄÊ±ºòÒ»¶¨ÒªĞ¡ĞÄ¡£",
  "pl" => "",
  "es" => "Si este archivo existe, todos los encabezados de un post a la lista son contrastados con las reglas. La primera regla que coincide gana. Ver README.access para sintaxis y ejemplos.",
  "pt" => "");

$txt_lists_hlp_footer = array (
  "fr" => "Remplissez ceci si vous voulez que tous les mails incluse quelquechose comme:<br>--<br>Pour vous désinscrire envoyez un message à coollist+unsubscribe@lists.domain.net.",
  "en" => "Fill this if you want every mail to have something like:<br>--<br>To unsubscribe send a mail to coollist+unsubscribe@lists.domain.net.",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Tragen Sie hier etwas ein, wenn jede Mail soetwas enthalten soll:<br><br>--<br>Um sich abzumelden, schicken Sie eine Mail an coollist+unsubscribe@lists.domain.net.",
  "zh" => "Èç¹ûÄãÏëÈÃËùÓĞµÄÓÊ¼ş¶¼Ìí¼ÓÀàËÆÓÚÕâÑùµÄÄÚÈİ£º<br>--<br>ÈçÒªÍË¶©Çë·¢ÓÊ¼ş¸ø coollist+unsubscribe@lists.domain.netµÄ»°£¬¿ÉÒÔÔÚÕâÀïÌîĞ´¡£",
  "pl" => "",
  "es" => "Complete aqu&iacute; si desea que todo correo tenga algo como:<br>--<br>Para desuscribirse envie un correo a coollist+unsuscribe@lists.domain.net.",
  "pt" => "");

$txt_lists_hlp_memorymailsize = array (
  "fr" => "Ici est spécifié le nombre d\'octet que peut faire un message en mémoire pour être préparé à être envoyé. Default à 16Ko.",
  "en" => "Here is specified in bytes how big a mail can be and still be prepared for sending in memory. It\'s greatly reducing the amount of write system calls to prepare it in memory before sending it, but can also lead to denial of service attacks. Default is 16k (16384 bytes).",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Hier k&ouml;nnen Sie in Byte spezifizieren, wie gross eine Mail sein darf, die immer noch im Speicher f&uuml;rs Senden vorbereitet werden soll. Eine Mail im Speicher vorzubereiten reduziert die Anzahl der \"write\" Systemaufrufe enorm, kann allerdings zu einer Denial of Service Situation f&uuml;hren. Standard ist 16k (16384 bytes).",
  "zh" => "ÕâÀïÓÃ±ÈÌØ±êÃ÷ÁË¶à´óµÄÓÊ¼şÔÚ·¢ËÍÖ®Ç°»ò·¢ËÍÊ±¿ÉÒÔ´æ·ÅÔÚÄÚ´æÖĞ¡£¸Ã¹¦ÄÜ¿ÉÒÔ¼«´óµÄ½µµÍÏµÍ³µÄI/O£¬µ«Í¬Ê±¿ÉÄÜ»Øµ¼ÖÂ¾Ü¾ø·şÎñÊ½¹¥»÷¡£Ä¬ÈÏ16k (16384 bytes)¡£",
  "pl" => "",
  "es" => "Aqu&iacute; se especifica en bytes qué tama&ntilde;o puede tener un correo y a&uacute;n ser preparado para enviar en memoria. El valor por defecto es 16K (16384 bytes).",
  "pt" => "");

$txt_lists_hlp_addtohdr = array (
  "fr" => "Lorsque ce drapeau est positionné un header To: incluant les adresses des destinataires sera ajouté aux messages sortants. La recommandation est de retiré les header To: grâce a l\'option delheaders au préalable (voir plus loin).",
  "en" => "When this flag is present, a To: header including the recipients emailaddress will be added to outgoing mail. Recommended usage is to remove existing To: headers with delheaders (see above) first.",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Falls Haken gesetzt ist, wird eine To: Zeile, die die EMailadresse des Empf&auml;ngers enth&auml;lt in jede ausgehende Mail eingef&uuml;gt. Falls Sie dies in Erw&auml;gung ziehen, sollten sie eventuelle To: Header mittels delheader (siehe oben) zuerst entfernen.",
  "zh" => "Èç¹ûÉèÖÃÁËÕâ¸öÑ¡Ïî£¬ËùÓĞÍâ·¢µÄÓÊ¼ş½«±»Ìí¼ÓÒ»¸ö°üº¬ÁËÊÕ¼şÈËÓÊ¼şµØÖ·µÄÒ³Í·¡£ÍÆ¼öµÄ×ö·¨ÊÇÉ¾µôTo:header(Çë²Î¿¼delheader)¡£",
  "pl" => "",
  "es" => "Cuando esta opci&oacute;n est&aacute; presente, un encabezado To: incluyendo las direcciones de los destinatarios será agregado al mail saliente. El uso recomendado es remover antes los To: existentes con &quot;Eliminar cabecera&quot; (ver arriba).",
  "pt" => "");

$txt_lists_hlp_notifysub = array (
  "fr" => "Si ce drapeau est drapeau est positionné, alors le(s) propriétaire(s) recevront les adresses des (dé)enregistrement à la liste.",
  "en" => "If this flag is present, the owner(s) will get a mail with the address of someone sub/unsubscribing to a mailinglist.",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Falls dieser Haken geetzt ist, kriegt der Listenbesitzer eine Mail mit der EMailadresse der Person, die sich an/abmelden will.",
  "zh" => "Èç¹û´æÔÚÕâ¸ö²ÎÊı£¬µ±ÓĞÈË¶©ÔÄ/ÍË¶©Õâ¸öÓÊ¼şÁĞ±íµÄÊ±ºò£¬¸ÃÓÊ¼şÁĞ±íµÄËùÓĞÕß»áÊÕµ½Ò»·âÌáÊ¾ÓÊ¼ş¡£",
  "pl" => "",
  "es" => "El propietario recibir&aacute; un mail con la direcci&oacute;n de alguien suscribi&eacute;ndose/desuscribi&eacute;ndose a una lista de correo.",
  "pt" => "");

$txt_lists_hlp_digestinterval = array (
  "fr" => "Cette valeur spécifie combien de secondes doivent passé avant que le nouveau digest soit déclanché. Le defaut est de 604800 secondes, ce qui correspond à 7 jours.",
  "en" => "This value specifies how many seconds will pass before the next digest is sent. Defaults to 604800 seconds, which is 7 days.",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Dieser Wert spezifiziert, wie viel Zeit (in Sekunden)  verstreichen m&uuml;ssen, bevor der n&auml;chste Digest gesendet wird. Standarf ist 604800 Sekunden, was 7 Tagen entspricht.",
  "zh" => "Õâ¸öÖµ±êÃ÷ÁËÕªÒªµÄ·¢ËÍÖÜÆÚ¡£Ä¬ÈÏ604800Ãë£¬Ò²¾ÍÊÇ7Ìì¡£",
  "pl" => "",
  "es" => "Este valor indica cu&aacute;ntos segundos pasar&aacute;n antes de que la siguiente recopilaci&oacute;n sea enviada. El valor por defecto es de 604800 segundos (7 dias).",
  "pt" => "");

$txt_lists_hlp_digestmaxmails = array (
  "fr" => "Cette valeur définie combien de mails peuvent s\'accumulés avant qu\'un digest soit envoyé. Defaut de 50 message.",
  "en" => "This file specifies how many mails can accumulate before digest sending is triggered. Defaults to 50 mails, meaning that if 50 mails arrive to the list before digestinterval have passed, the digest is delivered.",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Wie viele Mails d&uuml;rfen sich ansammeln, bevor ein Digest versendet wird. Standard ist 50 Mails, was bedeutet, dass ein Digest gesendet wird sobald 50 Mails eingegangen sind, auch bevor digestinterval verstrichen ist.",
  "zh" => "Õâ¸öÎÄ¼ş±êÃ÷ÁËÔÚ·¢ËÍÖÜÆÚÖ®Ç°ÓĞ¶àÉÙ·âÓÊ¼ş¿ÉÒÔ±»»º´æ¡£Ä¬ÈÏ50·â£¬ÕâÒâÎ¶×Å£¬ÔÚ·¢ËÍÖÜÆÚÖ®Ç°£¬Èç¹ûÓĞ50·âÓÊ¼şµ½´ïÁË£¬ÄÇÃ´¾Í·¢ËÍ¡£",
  "pl" => "",
  "es" => "Este archivo especifica cuantos correos pueden acumularse antes de enviar la recopilaci&oacute;n. Por defecto es 50, lo que significa que si llegan 50 correos a la lista antes de transcurrido el intervalo de recopilaci&oacute;n, dicha recopilaci&oacute;n es enviada.",
  "pt" => "");

$txt_lists_hlp_bouncelife = array (
  "fr" => "Ici est spécifié le nombre de secondes qu\'une adresse peut bouncer avant d\'être désinscrit. Le defaut est de 432000 secondes, ce qui est 5 jours.",
  "en" => "Here is specified for how long time in seconds an address can bounce before it\'s unsubscribed. Defaults to 432000 seconds, which is 5 days.",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Dieser Wert spezifiziert, wie lange (in Sekunden) eine Mail bouncen darf, bevor Sie von der Liste gestrichen wird. Standard ist 432000, was 5 Tagen entspricht.",
  "zh" => "ÕâÀïÖ¸Ã÷ÁËĞèÒª¶à³¤Ê±¼äÒ»·âÎŞ·¨È¡Ïû¶©ÔÄµÄÓÊ¼ş²ÅÄÜ¹»±»µ¯»Ø¡£Ä¬ÈÏÊÇ432000Ãë£¬Ò²¾ÍÊÇ5Ìì¡£",
  "pl" => "",
  "es" => "Aqu&iacute; se indica por cu&aacute;nto tiempo en segundos una direcci&oacute;n puede rebotar antes de su desuscripci&oacute;n. Por defecto es 432000 seg (5 dias).",
  "pt" => "");

$txt_lists_hlp_noarchive = array (
  "fr" => "Si ce drapeau est positionné, alors le mail n\'est pas sauvé dans l\'archive mais simplement effacé.",
  "en" => "If this flag exists, the mail won\'t be saved in the archive but simply deleted.",
  "hu" => "",
  "it" => "Se selezionato le email non verranno salvate nell\'archivio ma semplicemente cancellate.",
  "nl" => "",
  "ru" => "",
  "de" => "Falls Haken gesetzt ist, wird die Mail nicht archiviert, sondern einfach gel&ouml;scht.",
  "zh" => "µ±Õâ¸ö±êÖ¾´æÔÚµÄ»°£¬ÓÊ¼ş»á±»Ö±½ÓÉ¾³ı£¬¶ø²»»á±»±¸·İµ½¹éµµÎÄ¼şÖĞ¡£",
  "pl" => "",
  "es" => "Si se selecciona, el correo no ser&aacute; guardado en el archivo, simplemente ser&aacute; borrado.",
  "pt" => "");

$txt_lists_hlp_nosubconfirm = array (
  "fr" => "Si ce drapeau existe, aucun mail de confirmation est nécéssaire pour s\'inscrire à la liste. Ceci ne doit en pratique être utilisé que dans un réseau local. A UTILISER AVEC PRECAUTION!",
  "en" => "If this flag exists, no mail confirmation is needed to subscribe to the list. This should in principle never ever be used, but there is times on local lists etc. where this is useful. HANDLE WITH CARE!",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Falls Haken gesetzt ist, ist keine Best&auml;tigung per Mail erforderlich, um die Liste zu abonnieren. Diese sollte prinzipiell nie benutzt werden, kann aber in Einzelf&auml;llen sinnvoll sein.",
  "zh" => "Èç¹û´æÔÚÕâ¸ö±êÖ¾£¬Ìí¼Óµ½ÓÊ¼şÁĞ±íÊ±²»ĞèÒªÈ·ÈÏ¡£¾¡Á¿²»ÒªÊ¹ÓÃÕâ¸ö¹¦ÄÜ£¬ËäÈ»ÓĞÊ±ËüºÜÓĞÓÃ¡£Çë½÷É÷´¦ÀíÕâ¸ö±êÖ¾£¡",
  "pl" => "",
  "es" => "Si est&aacute; marcada, no ser&aacute; necesaria la confirmaci&oacute;n para suscribirse a una lista. Esto no deber&iacute;a usarse NUNCA, pero a veces es &uacute;til en listas locales. USAR CON CUIDADO!.",
  "pt" => "");

$txt_lists_hlp_noget = array (
  "fr" => "Si ce drapeau est positionné, récupérer des posts avec -get-N (par exemple maliste-get-12@mon-domaine.tld) est désactivé.",
  "en" => "If this file exists, then retrieving old posts with -get-N (for exemple mylist-get-12@my-domain.tld) is disabled",
  "hu" => "",
  "it" => "Se selezionato la possibilità di recuperare vecchi posts con -get-N è disabilitata.",
  "nl" => "",
  "ru" => "",
  "de" => "Falls Haken gesetzt ist, ist das Holen alter Posting mittels -get-N (Beispiel: mylist-get-12@my-domain.tld) nicht m&ouml;glich.",
  "zh" => "Èç¹ûÕâ¸öÎÄ¼ş´æÔÚµÄ»°£¬Ê¹ÓÃ-get-N²ÎÊı»Ö¸´µ½¾ÉÓÊ¼şÁĞ±íµÄ¹¦ÄÜ±»½ûÓÃ¡£",
  "pl" => "",
  "es" => "Si este archivo existe estar&aacute; deshabilitada la descarga de post antiguos con -get-N (ejemplo: milista-get-12@mi-dominio.tld).",
  "pt" => "");

$txt_lists_hlp_subonlyget = array (
  "fr" => "Si ce drapeau existe, récupéré des posts avec -get-N est possible uniquement pour les inscrits a la liste. Le drapeau \'noget\' est prioritaire.",
  "en" => "If this file exists, then retrieving old posts with -get-N is only possible for subscribers. The above mentioned \'noget\' have precedence.",
  "hu" => "",
  "it" => "",
  "nl" => "",
  "ru" => "",
  "de" => "Wenn diese Datei bereits existiert, haben nur Mitglieder der Liste die Möglichkeit, per -get-N alte Mailings einzusehen. Das oben stehende \'noget\' hat Vorrang.",
  "zh" => "Èç¹ûÕâ¸öÎÄ¼ş´æÔÚµÄ»°£¬Ê¹ÓÃ-get-N²ÎÊı»Ö¸´µ½¾ÉÓÊ¼şÁĞ±íµÄ¹¦ÄÜ½ö½öÔÚ¶©ÔÄÊ±ÓĞĞ§¡£Ç°ÃæµÄ\'noget\'ÓÅÏÈÖ´ĞĞ¡£",
  "pl" => "",
  "es" => "Si este archivo existe la descarga de post antiguos con -get-N s&oacute;lo es posible para suscriptos. Lo mencionado arriba \'noget\' tiene prioridad.",
  "pt" => "");

$txt_lists_hlp_verp = array (
  "fr" => "Active le support VERP.",
  "en" => "Enable VERP support.",
  "hu" => "",
  "it" => "Attiva il supporto VERP.",
  "nl" => "Activeer VERP ondersteuning",
  "ru" => "",
  "de" => "Aktiviere die VERP Unterstützung.",
  "zh" => "ÆôÓÃVERPÖ§³Ö¡£",
  "pl" => "",
  "es" => "Habilita soporte VERP",
  "pt" => "");

$txt_lists_hlp_maxverprecips = array (
  "fr" => "Combien de destinataire par mail doivent être envoyé au serveur SMTP. Le defaut est de 100.",
  "en" => "How many recipients pr. mail delivered to the smtp server. Defaults to 100.",
  "hu" => "",
  "it" => "Hoeveel ontvangers voordat mail wordt afgeleverd tot de smtp server. Standaard is 100.",
  "nl" => "",
  "ru" => "",
  "de" => "Wie viele Mails sollen auf einmal vom SMTP-Server versendet werden? Standard ist 100.",
  "zh" => "½ÓÊÕµ½µÄÓÊ¼şµÄ×î´óÈº·¢ÊıÁ¿¡£Ä¬ÈÏÊÇ100¡£",
  "pl" => "",
  "es" => "Cuantos destinatarios por mail se entregan al servidor SMTP. Por defecto son 100.",
  "pt" => "");

$txt_lists_hlp_notoccdenymails = array (
  "fr" => "Refuse les message si ni le To: ni le Cc: contient l\'une des adresses de la liste.",
  "en" => "Reject mails that don\'t have the list adress in the To: or Cc:.",
  "hu" => "",
  "it" => "Rifiuta tutte le email che non contengono l\indirizzo della lista su To: o Cc:.",
  "nl" => "Weiger mails die niet het addres van de lijst in To: of Cc: bevatten.",
  "ru" => "",
  "de" => "Emails zurückweisen, die nicht die Adresse der Mailingliste in To: oder Cc: haben.",
  "zh" => "¾ÜÊÕÃ»ÓĞÊÕ¼ş(»ò³­ËÍ)ÈËµÄÓÊ¼ş¡£",
  "pl" => "",
  "es" => "Rechaza los correos que no llevan la direcci&oacute;n de la lista en el To: o Cc:.",
  "pt" => "");

$txt_lists_hlp_noaccessdenymails = array (
  "fr" => "-",
  "en" => "-",
  "hu" => "",
  "it" => "-",
  "nl" => "",
  "ru" => "",
  "de" => "",
  "zh" => "-",
  "pl" => "",
  "es" => "",
  "pt" => "");

$txt_lists_hlp_nosubonlydenymails = array (
  "fr" => "-",
  "en" => "-",
  "hu" => "",
  "it" => "-",
  "nl" => "",
  "ru" => "",
  "de" => "",
  "zh" => "-",
  "pl" => "",
  "es" => "",
  "pt" => "");

$txt_lists_hlp_delimiter = array (
  "fr" => "Ne pas modifier a moins que vous sachiez vraiment ce que vous faites.",
  "en" => "Do not change unless you really know what you are doing.",
  "hu" => "",
  "it" => "Non cambiare a meno che tu non sappia quello che stai facendo.",
  "nl" => "Verander dit niet tenzij je weet waar je mee bezig bent",
  "ru" => "",
  "de" => "Hier bitte nichts ändern ausser Sie wissen genau, was tun...",
  "zh" => "²»Òª¸ü¸ÄËü£¬³ı·ÇÄúÖªµÀÄúÕıÔÚ×öÊ²Ã´¡£",
  "pl" => "",
  "es" => "No modificar a menos que realmente sepa lo que est&aacute; haciendo.",
  "pt" => "");

$txt_lists_hlp_relayhost = array (
  "fr" => "Serveur de mail utilisé pour les envois.",
  "en" => "Mail server used to send the messages.",
  "hu" => "",
  "it" => "Server mail usato per inviare messaggi.",
  "nl" => "Mail server om berichten mee te versturen",
  "ru" => "",
  "de" => "Der zum Versenden benutzte Mail-Server.",
  "zh" => "·¢ËÍÕâ·âÓÊ¼şµÄ·şÎñÆ÷¡£",
  "pl" => "",
  "es" => "Servidor de correo utilizado para enviar mensajes.",
  "pt" => "");
  
$txt_lists_hlp_webarchive = array (
  "fr" => "Activer l\'archivage",
  "en" => "Enable webarchive.",
  "hu" => "",
  "it" => "Selezionando questa opzione si avrà la generazione ogni ora dell\'archivio web.",
  "nl" => "Activeer web archief",
  "ru" => "",
  "de" => "Web-Archiv aktivieren.",
  "zh" => "ÆôÓÃ¹éµµ¹¦ÄÜ¡£",
  "pl" => "",
  "es" => "Habilitar archivo web",
  "pt" => "");

$txt_lists_hlp_spammode = array(
  "fr" => "Cache les adresses email pour éviter le spam.",
  "en" => "Hide email addresses to avoid spam.",
  "hu" => "TRANS Hide email addreses to avoid spam.",
  "it" => "TRANS Hide email addreses to avoid spam.",
  "nl" => "Verberg email adressen om spam te vermijden.",
  "ru" => "TRANS Hide email addreses to avoid spam.",
  "de" => "Email-Adressen verstecken, um Spam zu verhindern",
  "zh" => "Òş²ØÓÊ¼şµØÖ·¡£",
  "pl" => "TRANS Hide email addreses to avoid spam.",
  "es" => "Ocultar direcciones de email para evitar spam.",
  "pt" => "TRANS Hide email addreses to avoid spam.");


$txt_lists_hlp_rcfile = array (
  "fr" => "Insérez ici le modèle du code que vous voulez voirez insérer à votre archive web. Consultez la <a href=\'http://www.mhonarc.org/MHonArc/doc/resources/rcfile.html\' target=\'_blank\'>documentation</a> et voire les <a href=\'http://www.mhonarc.org/MHonArc/doc/app-rcfileexs.html\' target=\'_blank\'>examples</a>.",
  "en" => "Insert here the template\'s code that you want use for your web archive. Read <a href=\'http://www.mhonarc.org/MHonArc/doc/resources/rcfile.html\' target=\'_blank\'>documentation</a> and see <a href=\'http://www.mhonarc.org/MHonArc/doc/app-rcfileexs.html\' target=\'_blank\'>examples</a>.",
  "hu" => "",
  "it" => "Inserisci qui il codice del template che desideri usare per il tuo archivio web. Leggi la <a href=\'http://www.mhonarc.org/MHonArc/doc/resources/rcfile.html\' target=\'_blank\'>documentazione</a> e vedi gli <a href=\'http://www.mhonarc.org/MHonArc/doc/app-rcfileexs.html\' target=\'_blank\'>esempi</a>.",
  "nl" => "Geef hier de code in voor het template voor het webarchief. Zie <a href=\'http://www.mhonarc.org/MHonArc/doc/resources/rcfile.html\' target=\'_blank\'>documentatie</a> en zie voor<a href=\'http://www.mhonarc.org/MHonArc/doc/app-rcfileexs.html\' target=\'_blank\'>voorbeelden</a>.",
  "ru" => "",
  "de" => "Füge hier den Code des Templates ein, das Du für das Web-Archiv benutzen willst. Infos: <a href=\'http://www.mhonarc.org/MHonArc/doc/resources/rcfile.html\' target=\'_blank\'>Dokumentation</a> und <a href=\'http://www.mhonarc.org/MHonArc/doc/app-rcfileexs.html\' target=\'_blank\'>Beispiele</a>. ",
  "zh" => "ÔÚÕâÀïÊäÈëÄúÒªÊ¹ÓÃµÄ¹éµµÎÄ¼şÄ£°åµÄ´úÂë¡£²é¿´<a href=\'http://www.mhonarc.org/MHonArc/doc/resources/rcfile.html\' target=\'_blank\'>ÎÄµµ</a>»ò²é¿´<a href=\'http://www.mhonarc.org/MHonArc/doc/app-rcfileexs.html\' target=\'_blank\'>Ê¾Àı</a>¡£",
  "pl" => "",
  "es" => "Inserte aqu&iacute; el c&oacute;digo de la plantilla que desea usar para su archivo web. Lea <a href=\'http://www.mhonarc.org/MHonArc/doc/resources/rcfile.html\' target=\'_blank\'>documentation</a> y vea <a href=\'http://www.mhonarc.org/MHonArc/doc/app-rcfileexs.html\' target=\'_blank\'>examples</a>.",
  "pt" => "");

$txt_lists_hlp_recreatewa = array (
  "fr" => "",
  "en" => "Recreate all messages of webarchive. Use this only if you have changed the webarchive\'s template. NOTE: this works only if you have &quot;web archive&quot; checked.",
  "hu" => "",
  "it" => "Rigenera tutti i messaggi dell\'archivio web. Usalo solo se sono stati effettuati cambiamenti al file template. NOTA BENE: funziona solo se hai l\'&quot;archivio web&quot; attivato.",
  "nl" => "Recreeer alle berichten van webarchief. Gebruik dit alleen wanneer je de template van het webarchief hebt gewijzigd. LET OP: dit werkt alleen waanneer je &quot;web archive&quot; hebt aangevinkt",
  "ru" => "",
  "de" => "Wiederherstellen aller Nachrichten im Web-Archiv. Banutzung nur nach Änderung des Web-Archiv-Templates. INFO: Dies klappt nur, wenn &quot;Web-Archv&quot; aktiviert ist.",
  "zh" => "ÖØĞÂ´´½¨ËùÓĞ¹éµµÎÄ¼ş¡£Çë½ö½öÔÚÄúĞŞ¸ÄÁË¹éµµÎÄ¼şµÄÄ£°åÊ±²ÅÊ¹ÓÃÕâ¸ö¹¦ÄÜ¡£×¢Òâ£º½ö½öµ±ÄúÊ¹ÓÃ¹éµµÄ£Ê½µÄÊ±ºò£¬¸Ã²ÎÊı²ÅÓĞĞ§¡£",
  "pl" => "",
  "es" => "Recrea todos los mensajes del archivo web. Use esto s&oacute;lo si ha cambiado la plantilla del archivo web. NOTA: Esto sirve si tiene &quot;archivo web&quot; activado.",
  "pt" => "");

$txt_lists_hlp_deletewa = array (
  "fr" => "",
  "en" => "Delete all messages of webarchive. NOTE: this works only if you have &quot;web archive&quot; not checked.",
  "hu" => "",
  "it" => "Elimina tutto l\'archivio web. NOTA BENE: funziona solo se hai l\'&quot;archivio web&quot; disattivato.",
  "nl" => "Verwijder alle berichtten in het webarchief. LET OP: Dit werkt alleen wanneer je &quot;web archief&quot; niet hebt aangevinkt",
  "ru" => "",
  "de" => "Lösche alle Nachrichten im Webarchiv. INFO: Dies funktioniert nur, wenn &quot;Web-Archiv&quot; nicht aktiviert ist.",
  "zh" => "É¾³ıÁËËùÓĞµÄ¹éµµÎÄ¼ş¡£×¢Òâ£ºÖ»ÓĞµ±ÄúÓµÓĞ&quot¹éµµÎÄ¼şÊ±Ëü²Å»áÖ´ĞĞ;¹éµµÎÄ¼ş&quot;Ã»ÓĞ±»¼ì²é¡£",
  "pl" => "Usuñ wszystkie wiadomo¶ci z archiwum na stronie. UWAGA: Zadzia³a tylko je¶li nie zaznaczone archiwum na www.",
  "es" => "Elimina todos los mensajes del archivo web. NOTA: Esto sirve si tiene &quot;archivo web&quot; desactivado.",
  "pt" => "");

$txt_lists_email_subject = array (
  "fr" => "",
  "en" => "List #name#@#domain# successfully created!",
  "hu" => "",
  "it" => "Lista #name#@#domain# creata con successo!",
  "nl" => "Lijst #name#@#domain# succesvol gecreeerd!",
  "ru" => "",
  "de" => "Liste #name#Q#domain# erfolgreich angelegt!",
  "zh" => "³É¹¦´´½¨ÁË#name#@#domain#ÁĞ±í£¡",
  "pl" => "Utworzono listê #name#@#domain#",
  "es" => "Lista #name#@#domain# creada con &eacute;xito!.",
  "pt" => "");
  
$txt_lists_email_msg = array (
  "fr" => "",
  "en" => "You can control your mailing list by email or by control panel.
You can send a email to the following addresses to execute the listed command:
  
  #name#-help@#domain# -> return the most common list's commands
  #name#-list@#domain# -> return the subscribers's list
  #name#-get-N@#domain# -> return the N list's message
  #name#-owner@#domain# -> you can contact the list's owner by this address
  #name#-unsubscribe@#domain# -> unsubscribe the email's sender from the list
  #name#-subscribe@#domain# -> subscribe the email's sender to the list
  #name#-subscribe-digest@#domain# -> subscribe the email's sender to the list's digest version
  #name#-subscribe-nomail@#domain# -> subscribe the email's sender to the list's nomail version
  
For more informations go to mlmmj official web site at http://mlmmj.mmj.dk",
  "hu" => "",
  "it" => "La mailing list può essere controllata via email o tramite il pannello di controllo.
Di seguito gli indirizzi ai quali puoi inviare una email per eseguire il rispettivo comando:

  #name#-help@#domain# -> restituisce la lista dei comandi più comuni
  #name#-list@#domain# -> restituisce la lista degli iscritti alla lista
  #name#-get-N@#domain# -> restituisce il messaggio N della lista
  #name#-owner@#domain# -> permette di inviare una email al proprietario della lista
  #name#-unsubscribe@#domain# -> elimina l'indirizzo email del mittente dalla lista
  #name#-subscribe@#domain# -> iscrive l'indirizzo email del mittente alla lista
  #name#-subscribe-digest@#domain# -> iscrive l'indirizzo email del mittente alla versione digest della lista
  #name#-subscribe-nomail@#domain# -> iscrive l'indirizzo email del mittente alla versione nomail della lista
  
Per ulteriori informazioni visita il sito ufficiale di mlmmj all'indirizzo http://mlmmj.mmj.dk",
  "nl" => "You can control your mailing list by email or by control panel.
Je kan een email naar het volgende adres om de opdracht uit te voeren:
  
  #name#-help@#domain# ->toon de meest voorkomende opdrachten
  #name#-list@#domain# -> toon de lijst van abonnees
  #name#-get-N@#domain# -> toon  N list's bericht
  #name#-owner@#domain# -> neem contact op met de eigenaar van de lijst van dit adress.
  #name#-unsubscribe@#domain# -> deabbonneer het email adress van de afzender van de lijst
  #name#-subscribe@#domain# -> abonneer de afzender van het bericht tot de lijst
  #name#-subscribe-digest@#domain# -> abonneer de afzender van het email bericht tot de digest versie van de lijst
  #name#-subscribe-nomail@#domain# -> abonneer de afzender van het email bericht tot de digest versie van de lijst 
  
Voor meer informatie bezoek de officiele mlmmj website http://mlmmj.mmj.dk",
  "ru" => "",
  "de" => "",
  "zh" => "Äú¿ÉÒÔÍ¨¹ıÓÊ¼ş»òÕß¿ØÖÆÃæ°åÀ´¹ÜÀíÄúµÄÓÊ¼şÁĞ±í¡£
Äú¿ÉÒÔ·¢ËÍÓÊ¼şµ½ÏÂÁĞÓÊÏäÀ´Ö´ĞĞÏàÓ¦µÄÃüÁî£º

¡¡#name#-help@#domain# -> ·µ»Ø³£ÓÃµÄÃüÁî
  #name#-list@#domain# -> ·µ»Ø¶¨ÔÄÕßÁĞ±í
  #name#-get-N@#domain# -> ·µ»ØµÚNÌõÏûÏ¢£¿
  #name#-owner@#domain# -> Í¨¹ıÕâ¸öµØÖ·¿ÉÒÔÁªÏµµ½ÓÊ¼şÁĞ±íµÄËùÓĞÕß
  #name#-unsubscribe@#domain# -> ´ÓÓÊ¼şÁĞ±íÖĞÉ¾³ıÒ»¸ö¶©ÔÄÕß
  #name#-subscribe@#domain# -> Ìí¼ÓÒ»¸ö¶©ÔÄÕßµ½ÓÊ¼şÁĞ±í
  #name#-subscribe-digest@#domain# -> ½«¶©ÔÄÕßÌí¼Óµ½ÓÊ¼şÁĞ±íµÄÕªÒªÖĞ
  #name#-subscribe-nomail@#domain# -> ½«¶©ÔÄÕßÌí¼Óµ½ÓÊ¼şÁĞ±íÖĞµÄ²»·¢ËÍÓÊ¼şµÄÁĞ±íÖĞ
¸ü¶àĞÅÏ¢Çë·ÃÎÊmlmmj¹Ù·½ÍøÕ¾http://mlmmj.mmj.dk",
  "pl" => "Mo¿esz kontrolowaæ swoj± listê mailingow± przy pomocy maila lub panela sterowania DTC.
	Mo¿esz wys³aæ maila na jeden z poni¿szych adresów aby wykonaæ komendy na li¶cie:
  
  #name#-help@#domain# -> zwraca pomoc i litê dostêpnych komend
  #name#-list@#domain# -> zwraca listê osób zapisanych
  #name#-get-N@#domain# -> zwraca ilo¶æ N wiadomo¶ci z listy
  #name#-owner@#domain# -> Przez ten adres mozesz kontaktowaæ siê z list± jako w³a¶ciciel
  #name#-unsubscribe@#domain# -> Wypisuje z listy maila, z którego wys³ano wiadomo¶æ
  #name#-subscribe@#domain# -> Zapisuje na listê adres, z którego wys³ano maila  
  #name#-subscribe-digest@#domain# -> zapisuje adres na li¶cie w wersji skróconej (zbiorczy mail raz dziennie)
  #name#-subscribe-nomail@#domain# -> Zapisuje na listê adres z ktorego wys³ano maila (je¶li by³ na skróconej)
    
Dla uzyskania wiêkszej ilo¶ci informacji prosimy skorzystaæ z oficjalej strony mlmmj http://mlmmj.mmj.dk
  
  
  ",
  "es" => "Ud. puede controlar su lista de correo por mail o desde el panel de control.
Puede enviar un mail a las siguientes direcciones para ejecutar los comandos listados:

 #name#-help@#domain# -> devuelve la lista de comandos m&aacute;s comunes
 #name#-list@#domain# -> devuelve las lista de suscriptos
 #name#-get-N@#domain# -> devuelve el mensaje de la lista N
 #name#-owner@#domain# -> Ud. puede contactar al propietario de la lista con esta direcci&oacute;n
 #name#-unsubscribe@#domain# -> desuscribe de la lista al remitente del correo
 #name#-subscribe@#domain# -> suscribe en la lista al remitente
 #name#-subscribe-digest@#domain# -> subscribe al remitente a la versi&oacute;n digest de la lista
 #name#-subscribe-nomail@#domain# -> subscribe al remitente a la versi&oacute;n nomail de la lista

Para m&aacute;s informaci&oacute;n vaya al sitio oficial de mlmmj en http://mlmmj.mmj.df.",
  "pt" => "");

?>
