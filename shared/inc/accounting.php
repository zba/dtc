...
function sum_http($webname)
{
	...
	for($i=0;$i<$num_rows;$i++)
    {
       $subdomain_name = mysql_result($result,$i,"subdomain_name");
       $db_select_name = "access_".$subdomain_name."_".$db_webname;
	   $last_run = "SELECT MAX(last_run) AS last FROM http_accounting WHERE vhost='".$subdomain_name."'";
	   ...
	   for($l=0;$l<mysql_num_rows($result_l);$l++)
	   {
			...
			if($last_run_st==NULL || $last_run_st==0)
	   		{
				...
				$query_insert = "INSERT INTO http_accounting (vhost,last_run,month,year) VALUES ('".$subdomain_name."','".$last_run_st."', ".date("m",time()).",".date("Y",time()).")";
			...
			elseif(strcmp(date("m.Y",$last_run_st),date("m.Y",time())))
			{
				...
				$query_insert = "INSERT INTO http_accounting (vhost,last_run,month,year) VALUES ('".$subdomain_name."','".time()."',".date("m",time()).",".date("Y",time()).")";
				mysql_query($query_insert)or die("Cannot execute query \"$query_insert\"");
			}
				if(mysql_table_exists("apachelogs",$db_select_name))
				{
					...
					$query_insert_bytes = "UPDATE http_accounting set
...
function sum_ftp($webname)
{
	...
	$last_run = "SELECT MAX(last_run) AS last FROM ftp_accounting WHERE sub_domain='".$webname."'";
	$result_l = mysql_query($last_run) or die("Cannot execute query \"$last_run\"");
 	$last_run_st = mysql_result($result_l,0,"last");
	if($last_run_st==NULL || $last_run_st==0)
	{
		echo "Create first entry for ".$webname."<br />";
		$query_insert = "INSERT INTO ftp_accounting (sub_domain,last_run,month,year) VALUES ('".$webname."','".$last_run_st."', ".date("m",time()).",".date("Y",time()).")";
		mysql_query($query_insert)or die("Cannot execute query \"$query_insert\"");
		$last_run_st=0;
	}
		elseif(strcmp(date("m.Y",$last_run_st),date("m.Y",time())))
	{
		...
		$query_insert = "INSERT INTO ftp_accounting (sub_domain,last_run,month,year) VALUES ('".$webname."','".time()."',".date("m",time()).",".date("Y",time()).")";
		mysql_query($query_insert)or die("Cannot execute query \"$query_insert\"");
	}
	$query = "SELECT SUM(dl_bytes) AS dl_amount, SUM(ul_bytes) AS ul_amount FROM $pro_mysql_ftp_table WHERE hostname='".$webname."'";
    mysql_select_db("dtc");
	...
	$query_insert = "UPDATE ftp_accounting set transfer=transfer+".$total_ftp_amount."+0, last_run=".time()." WHERE last_run=".$last_run_st;
	mysql_query($query_insert) or die("Cannot execute query \"$query_insert\"");
}
